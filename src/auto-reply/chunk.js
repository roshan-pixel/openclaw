...
function resolveChunkLimitForProvider(cfgSection, accountId) {
    ...
    if (accounts && typeof accounts === "object") {
        ...
        if (typeof (direct === null || direct === void 0 ? void 0 : direct.textChunkLimit) === "number") {
            return direct.textChunkLimit;
        }
        var matchKey = Object.keys(accounts).find(function (key) { return key.toLowerCase() === normalizedAccountId.toLowerCase(); });
...
function resolveTextChunkLimit(cfg, provider, accountId, opts) {
    var fallback = typeof (opts === null || opts === void 0 ? void 0 : opts.fallbackLimit) === "number" && opts.fallbackLimit > 0
        ? opts.fallbackLimit
        : DEFAULT_CHUNK_LIMIT;
    var providerOverride = (function () {
        ...
        var providerConfig = ((_a = channelsConfig === null || channelsConfig === void 0 ? void 0 : channelsConfig[provider]) !== null && _a !== void 0 ? _a : cfg === null || cfg === void 0 ? void 0 : cfg[provider]);
...
function chunkByParagraph(text, limit, opts) {
    ...
    if (limit <= 0)
        return [text];
    var splitLongParagraphs = (opts === null || opts === void 0 ? void 0 : opts.splitLongParagraphs) !== false;
...
function splitByNewline(text, isSafeBreak) {
    if (isSafeBreak === void 0) { isSafeBreak = function () { return true; }; }
    var lines = [];
    var start = 0;
...
function chunkText(text, limit) {
    ...
    while (remaining.length > limit) {
        ...
        var _a = scanParenAwareBreakpoints(window_1), lastNewline = _a.lastNewline, lastWhitespace = _a.lastWhitespace;
    ...
    while (remaining.length > limit) {
        ...
        if (initialFence) {
            ...
            if (maxIdxIfNeedNewline <= 0) {
                ...
                var minProgressIdx = Math.min(remaining.length, initialFence.start + initialFence.openLine.length + 2);
        ...
        if (fenceToSplit) {
            ...
            rawChunk = rawChunk.endsWith("\n") ? "".concat(rawChunk).concat(closeLine) : "".concat(rawChunk, "\n").concat(closeLine);
...
function pickSafeBreakIndex(window, spans) {
    ...
    }), lastNewline = _a.lastNewline, lastWhitespace = _a.lastWhitespace;
    if (lastNewline > 0)
        return lastNewline;
    if (lastWhitespace > 0)
...
function scanParenAwareBreakpoints(window, isAllowed) {
    if (isAllowed === void 0) { isAllowed = function () { return true; }; }
    var lastNewline = -1;
