...
function defineChatCommand(command) {
    ...
    var aliases = ((_a = command.textAliases) !== null && _a !== void 0 ? _a : (command.textAlias ? [command.textAlias] : []))
        .map(function (alias) { return alias.trim(); })
        .filter(Boolean);
    var scope = (_b = command.scope) !== null && _b !== void 0 ? _b : (command.nativeName ? (aliases.length ? "both" : "native") : "text");
    var acceptsArgs = (_c = command.acceptsArgs) !== null && _c !== void 0 ? _c : Boolean((_d = command.args) === null || _d === void 0 ? void 0 : _d.length);
    var argsParsing = (_e = command.argsParsing) !== null && _e !== void 0 ? _e : (((_f = command.args) === null || _f === void 0 ? void 0 : _f.length) ? "positional" : "none");
    return {
        key: command.key,
...
function registerAlias(commands, key) {
    ...
    var command = commands.find(function (entry) { return entry.key === key; });
    if (!command) {
        throw new Error("registerAlias: unknown command key: ".concat(key));
    }
    var existing = new Set(command.textAliases.map(function (alias) { return alias.trim().toLowerCase(); }));
...
function buildChatCommands() {
    var commands = __spreadArray([
            ...
            args: [
                {
                    ...
                    type: "string",
                    choices: function (_a) {
                        var provider = _a.provider, model = _a.model;
        ...
        .filter(function (dock) { return dock.capabilities.nativeCommands; })
        .map(function (dock) { return defineDockCommand(dock); }), true);
...
function getNativeCommandSurfaces() {
    ...
    cachedNativeCommandSurfaces = new Set((0, dock_js_1.listChannelDocks)()
        .filter(function (dock) { return dock.capabilities.nativeCommands; })
        .map(function (dock) { return dock.id; }));
