...
function buildSelectionFromExplicit(params) {
    ...
    if (params.allowedModelKeys.size > 0 && !params.allowedModelKeys.has(key))
        return undefined;
    var isDefault = resolved.ref.provider === params.defaultProvider && resolved.ref.model === params.defaultModel;
    return __assign({ provider: resolved.ref.provider, model: resolved.ref.model, isDefault: isDefault }, (resolved.alias ? { alias: resolved.alias } : undefined));
}
function applySelectionToSession(params) {
    var selection = params.selection, sessionEntry = params.sessionEntry, sessionStore = params.sessionStore, sessionKey = params.sessionKey, storePath = params.storePath;
...
function applyResetModelOverride(params) {
    return __awaiter(this, void 0, void 0, function () {
        var rawBody, _a, tokens, first, second, catalog, allowed, allowedModelKeys, providers, _i, allowedModelKeys_1, key, slash, resolveSelection, selection, consumed, composite, resolved, resolved, allowFuzzy, cleanedBody;
        var _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    if (!rawBody)
                        return [2 /*return*/, {}];
                    _a = splitBody(rawBody), tokens = _a.tokens, first = _a.first, second = _a.second;
                    if (!first)
                        return [2 /*return*/, {}];
                ...
                case 1:
                    ...
                    if (allowedModelKeys.size === 0)
                        return [2 /*return*/, {}];
                    providers = new Set();
                    for (_i = 0, allowedModelKeys_1 = allowedModelKeys; _i < allowedModelKeys_1.length; _i++) {
                    ...
                    if (providers.has((0, model_selection_js_1.normalizeProviderId)(first)) && second) {
                        composite = "".concat((0, model_selection_js_1.normalizeProviderId)(first), "/").concat(second);
                    ...
                    if (!selection) {
                        ...
                        allowFuzzy = providers.has((0, model_selection_js_1.normalizeProviderId)(first)) || first.trim().length >= 6;
                        if (allowFuzzy) {
                            selection = resolved.selection;
                            if (selection)
                                consumed = 1;
                    ...
                    params.sessionCtx.BodyStripped = (0, inbound_sender_meta_js_1.formatInboundBodyWithSenderMeta)({
                        ctx: params.ctx,
                        body: cleanedBody,
                    });
