...
var resolveAuthLabel = function (provider_1, cfg_1, modelsPath_1, agentDir_1) {
    ...
    return __awaiter(void 0, __spreadArray([provider_1, cfg_1, modelsPath_1, agentDir_1], args_1, true), void 0, function (provider, cfg, modelsPath, agentDir, mode) {
        var formatPath, store, order, providerKey, lastGood, nextProfileId, now, formatUntil, profileId, profile, configProfile, missing, more, exp_1, display, label, exp, labels, envKey, isOAuthEnv, label, customKey;
        var _a, _b;
        if (mode === void 0) { mode = "compact"; }
        return __generator(this, function (_c) {
            ...
            order = (0, model_auth_js_1.resolveAuthProfileOrder)({ cfg: cfg, store: store, provider: provider });
            ...
            if (order.length > 0) {
                if (mode === "compact") {
                    ...
                    configProfile = (_b = (_a = cfg.auth) === null || _a === void 0 ? void 0 : _a.profiles) === null || _b === void 0 ? void 0 : _b[profileId];
                    missing = !profile ||
                        ((configProfile === null || configProfile === void 0 ? void 0 : configProfile.provider) && configProfile.provider !== profile.provider) ||
                    ...
                    if (missing)
                        return [2 /*return*/, { label: "".concat(profileId, " missing").concat(more), source: "" }];
                    if (profile.type === "api_key") {
                        return [2 /*return*/, {
                                label: "".concat(profileId, " api-key ").concat(maskApiKey(profile.key)).concat(more),
                                source: "",
                            }];
                    }
                    if (profile.type === "token") {
                        exp_1 = typeof profile.expires === "number" &&
                            ...
                            : "";
                        return [2 /*return*/, {
                                label: "".concat(profileId, " token ").concat(maskApiKey(profile.token)).concat(exp_1).concat(more),
                                source: "",
                            }];
                    }
                    display = (0, auth_profiles_js_1.resolveAuthProfileDisplayLabel)({ cfg: cfg, store: store, profileId: profileId });
                    label = display === profileId ? profileId : display;
                    exp = typeof profile.expires === "number" &&
                        Number.isFinite(profile.expires) &&
                        profile.expires > 0
                        ? profile.expires <= now
                            ? " expired"
                            : " exp ".concat(formatUntil(profile.expires))
                        : "";
                    return [2 /*return*/, { label: "".concat(label, " oauth").concat(exp).concat(more), source: "" }];
                }
                labels = order.map(function (profileId) {
                    var _a, _b, _c, _d;
                    var profile = store.profiles[profileId];
                    var configProfile = (_b = (_a = cfg.auth) === null || _a === void 0 ? void 0 : _a.profiles) === null || _b === void 0 ? void 0 : _b[profileId];
                    var flags = [];
                    ...
                    if ((0, auth_profiles_js_1.isProfileInCooldown)(store, profileId)) {
                        var until = (_d = (_c = store.usageStats) === null || _c === void 0 ? void 0 : _c[profileId]) === null || _d === void 0 ? void 0 : _d.cooldownUntil;
                    ...
                    if (profile.type === "token") {
                        if (typeof profile.expires === "number" &&
                            Number.isFinite(profile.expires) &&
                            profile.expires > 0) {
                            flags.push(profile.expires <= now ? "expired" : "exp ".concat(formatUntil(profile.expires)));
                        }
                        var suffix_3 = flags.length > 0 ? " (".concat(flags.join(", "), ")") : "";
                        return "".concat(profileId, "=token:").concat(maskApiKey(profile.token)).concat(suffix_3);
                    }
                ...
                return [2 /*return*/, {
                        label: labels.join(", "),
                        source: "auth-profiles.json: ".concat(formatPath((0, auth_profiles_js_1.resolveAuthStorePathForDisplay)(agentDir))),
                    }];
            }
            envKey = (0, model_auth_js_1.resolveEnvApiKey)(provider);
            if (envKey) {
                isOAuthEnv = envKey.source.includes("ANTHROPIC_OAUTH_TOKEN") ||
...
var resolveProfileOverride = function (params) {
    ...
    if (profile.provider !== params.provider) {
        return {
            error: "Auth profile \"".concat(raw, "\" is for ").concat(profile.provider, ", not ").concat(params.provider, "."),
        };
    }
