...
function isRunAlreadyQueued(run, items, allowPromptFallback) {
    var _a;
    if (allowPromptFallback === void 0) { allowPromptFallback = false; }
    ...
    if (messageId) {
        return items.some(function (item) { var _a; return ((_a = item.messageId) === null || _a === void 0 ? void 0 : _a.trim()) === messageId && hasSameRouting(item); });
    }
    if (!allowPromptFallback)
        return false;
    return items.some(function (item) { return item.prompt === run.prompt && hasSameRouting(item); });
}
function enqueueFollowupRun(key, run, settings, dedupeMode) {
    if (dedupeMode === void 0) { dedupeMode = "message-id"; }
    var queue = (0, state_js_1.getFollowupQueue)(key, settings);
    var dedupe = dedupeMode === "none"
        ? undefined
        : function (item, items) {
            return isRunAlreadyQueued(item, items, dedupeMode === "prompt");
        };
    // Deduplicate: skip if the same message is already queued.
    if ((0, queue_helpers_js_1.shouldSkipQueueItem)({ item: run, items: queue.items, dedupe: dedupe }))
        return false;
