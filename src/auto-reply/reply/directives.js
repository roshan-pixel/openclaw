...
var extractSimpleDirective = function (body, names) {
    var namePattern = names.map(escapeRegExp).join("|");
    var match = body.match(new RegExp("(?:^|\\s)\\/(?:".concat(namePattern, ")(?=$|\\s|:)(?:\\s*:\\s*)?"), "i"));
...
function extractThinkDirective(body) {
    ...
    var extracted = extractLevelDirective(body, ["thinking", "think", "t"], thinking_js_1.normalizeThinkLevel);
    return {
...
function extractVerboseDirective(body) {
    if (!body)
        return { cleaned: "", hasDirective: false };
    var extracted = extractLevelDirective(body, ["verbose", "v"], thinking_js_1.normalizeVerboseLevel);
    return {
        cleaned: extracted.cleaned,
...
function extractNoticeDirective(body) {
    if (!body)
        return { cleaned: "", hasDirective: false };
    var extracted = extractLevelDirective(body, ["notice", "notices"], thinking_js_1.normalizeNoticeLevel);
    return {
        cleaned: extracted.cleaned,
...
function extractElevatedDirective(body) {
    if (!body)
        return { cleaned: "", hasDirective: false };
    var extracted = extractLevelDirective(body, ["elevated", "elev"], thinking_js_1.normalizeElevatedLevel);
    return {
        cleaned: extracted.cleaned,
...
function extractReasoningDirective(body) {
    if (!body)
        return { cleaned: "", hasDirective: false };
    var extracted = extractLevelDirective(body, ["reasoning", "reason"], thinking_js_1.normalizeReasoningLevel);
    return {
        cleaned: extracted.cleaned,
...
Object.defineProperty(exports, "extractExecDirective", { enumerable: true, get: function () { return directive_js_1.extractExecDirective; } });
