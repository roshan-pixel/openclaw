...
function parseLineDirectives(payload) {
    ...
    var lineData = __assign({}, (_a = result.channelData) === null || _a === void 0 ? void 0 : _a.line);
    ...
    if (quickRepliesMatch) {
        var options = quickRepliesMatch[1]
            ...
            .filter(Boolean);
        if (options.length > 0) {
            lineData.quickReplies = __spreadArray(__spreadArray([], (lineData.quickReplies || []), true), options, true);
        }
    ...
    if (locationMatch && !lineData.location) {
        var parts = locationMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 4) {
            var title = parts[0], address = parts[1], latStr = parts[2], lonStr = parts[3];
    ...
    if (confirmMatch && !lineData.templateMessage) {
        var parts = confirmMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 3) {
            var question = parts[0], yesPart = parts[1], noPart = parts[2];
            // Parse yes_label:yes_data format
            var _b = yesPart.includes(":")
                ? yesPart.split(":").map(function (s) { return s.trim(); })
                : [yesPart, yesPart.toLowerCase()], yesLabel = _b[0], yesData = _b[1];
            var _c = noPart.includes(":")
                ? noPart.split(":").map(function (s) { return s.trim(); })
                : [noPart, noPart.toLowerCase()], noLabel = _c[0], noData = _c[1];
    ...
    if (buttonsMatch && !lineData.templateMessage) {
        var parts = buttonsMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 3) {
            var title = parts[0], bodyText = parts[1], actionsStr = parts[2];
    ...
    if (mediaPlayerMatch && !lineData.flexMessage) {
        var parts = mediaPlayerMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 1) {
            var title = parts[0], artist = parts[1], source = parts[2], imageUrl = parts[3], statusStr = parts[4];
            var isPlaying = (statusStr === null || statusStr === void 0 ? void 0 : statusStr.toLowerCase()) === "playing";
            // LINE requires HTTPS URLs for images - skip local/HTTP URLs
            var validImageUrl = (imageUrl === null || imageUrl === void 0 ? void 0 : imageUrl.startsWith("https://")) ? imageUrl : undefined;
    ...
    if (eventMatch && !lineData.flexMessage) {
        var parts = eventMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 2) {
            var title = parts[0], date = parts[1], time = parts[2], location_1 = parts[3], description = parts[4];
            ...
            lineData.flexMessage = {
                altText: "\uD83D\uDCC5 ".concat(title, " - ").concat(date).concat(time ? " ".concat(time) : ""),
                contents: card,
            };
        }
    ...
    if (appleTvMatch && !lineData.flexMessage) {
        var parts = appleTvMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 1) {
            var deviceName = parts[0], status_1 = parts[1];
    ...
    if (agendaMatch && !lineData.flexMessage) {
        var parts = agendaMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 2) {
            var title = parts[0], eventsStr = parts[1];
    ...
    if (deviceMatch && !lineData.flexMessage) {
        var parts = deviceMatch[1].split("|").map(function (s) { return s.trim(); });
        if (parts.length >= 1) {
            var deviceName = parts[0], deviceType = parts[1], status_2 = parts[2], controlsStr = parts[3];
            var deviceKey_1 = toSlug(deviceName || "device");
            var controls = controlsStr
                ? controlsStr.split(",").map(function (ctrlStr) {
                    var _a = ctrlStr.split(":").map(function (s) { return s.trim(); }), label = _a[0], data = _a[1];
