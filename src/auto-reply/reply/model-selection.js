...
function boundedLevenshteinDistance(a, b, maxDistance) {
    ...
    var prev = Array.from({ length: bLen + 1 }, function (_, idx) { return idx; });
...
function resolveModelOverrideFromEntry(entry) {
    var _a, _b;
    var model = (_a = entry === null || entry === void 0 ? void 0 : entry.modelOverride) === null || _a === void 0 ? void 0 : _a.trim();
    if (!model)
        return null;
    var provider = ((_b = entry === null || entry === void 0 ? void 0 : entry.providerOverride) === null || _b === void 0 ? void 0 : _b.trim()) || undefined;
...
function scoreFuzzyMatch(params) {
    ...
    var fragmentVariants = FUZZY_VARIANT_TOKENS.filter(function (token) { return fragment.includes(token); });
    var modelVariants = FUZZY_VARIANT_TOKENS.filter(function (token) { return modelLower.includes(token); });
    var variantMatchCount = fragmentVariants.filter(function (token) { return modelLower.includes(token); }).length;
...
function createModelSelectionState(params) {
    return __awaiter(this, void 0, void 0, function () {
        var cfg, agentCfg, sessionEntry, sessionStore, sessionKey, parentSessionKey, storePath, defaultProvider, defaultModel, provider, model, hasAllowlist, initialStoredOverride, hasStoredOverride, needsModelCatalog, allowedModelKeys, allowedModelCatalog, modelCatalog, resetModelOverride, allowed, overrideProvider, overrideModel, key, updated, storedOverride, candidateProvider, key, ensureAuthProfileStore, store, profile, providerKey, defaultThinkingLevel, resolveDefaultThinkingLevel;
        var _this = this;
        var _a, _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    cfg = params.cfg, agentCfg = params.agentCfg, sessionEntry = params.sessionEntry, sessionStore = params.sessionStore, sessionKey = params.sessionKey, parentSessionKey = params.parentSessionKey, storePath = params.storePath, defaultProvider = params.defaultProvider, defaultModel = params.defaultModel;
                    ...
                    if (!needsModelCatalog) return [3 /*break*/, 2];
                ...
                case 1:
                    ...
                    _c.label = 2;
                case 2:
                    if (!(sessionEntry && sessionStore && sessionKey && hasStoredOverride)) return [3 /*break*/, 5];
                    overrideProvider = ((_a = sessionEntry.providerOverride) === null || _a === void 0 ? void 0 : _a.trim()) || defaultProvider;
                    overrideModel = (_b = sessionEntry.modelOverride) === null || _b === void 0 ? void 0 : _b.trim();
                    if (!overrideModel) return [3 /*break*/, 5];
                    key = (0, model_selection_js_1.modelKey)(overrideProvider, overrideModel);
                    if (!(allowedModelKeys.size > 0 && !allowedModelKeys.has(key))) return [3 /*break*/, 5];
                ...
                case 5:
                    ...
                    if (storedOverride === null || storedOverride === void 0 ? void 0 : storedOverride.model) {
                    ...
                    if (!(sessionEntry && sessionStore && sessionKey && sessionEntry.authProfileOverride)) return [3 /*break*/, 8];
                ...
                case 6:
                    ...
                    if (!(!profile || (0, model_selection_js_1.normalizeProviderId)(profile.provider) !== providerKey)) return [3 /*break*/, 8];
                ...
                case 7:
                    ...
                    _c.label = 8;
                case 8:
                    resolveDefaultThinkingLevel = function () { return __awaiter(_this, void 0, void 0, function () {
                        ...
                        return __generator(this, function (_b) {
                            switch (_b.label) {
                                case 0:
                                    ...
                                    if (!(!catalogForThinking || catalogForThinking.length === 0)) return [3 /*break*/, 2];
                                    return [4 /*yield*/, (0, model_catalog_js_1.loadModelCatalog)({ config: cfg })];
                                case 1:
                                ...
                                case 2:
                                    ...
                                    defaultThinkingLevel =
                                        (_a = resolved !== null && resolved !== void 0 ? resolved : agentCfg === null || agentCfg === void 0 ? void 0 : agentCfg.thinkingDefault) !== null && _a !== void 0 ? _a : "off";
                                    return [2 /*return*/, defaultThinkingLevel];
                            }
                        });
                    }); };
...
function resolveModelDirectiveSelection(params) {
    var raw = params.raw, defaultProvider = params.defaultProvider, defaultModel = params.defaultModel, aliasIndex = params.aliasIndex, allowedModelKeys = params.allowedModelKeys;
    var rawTrimmed = raw.trim();
    var rawLower = rawTrimmed.toLowerCase();
    var pickAliasForKey = function (provider, model) { var _a; return (_a = aliasIndex.byKey.get((0, model_selection_js_1.modelKey)(provider, model))) === null || _a === void 0 ? void 0 : _a[0]; };
    var buildSelection = function (provider, model) {
        var alias = pickAliasForKey(provider, model);
        return __assign({ provider: provider, model: model, isDefault: provider === defaultProvider && model === defaultModel }, (alias ? { alias: alias } : undefined));
    };
    var resolveFuzzy = function (params) {
        var fragment = params.fragment.trim().toLowerCase();
        if (!fragment)
            return {};
        var providerFilter = params.provider ? (0, model_selection_js_1.normalizeProviderId)(params.provider) : undefined;
        var candidates = [];
        ...
        if (!params.provider) {
            ...
            for (var _a = 0, _b = aliasIndex.byAlias.entries(); _a < _b.length; _a++) {
                var _c = _b[_a], aliasKey = _c[0], entry = _c[1];
            ...
            var _loop_1 = function (match) {
                ...
                if (!allowedModelKeys.has(key))
                    return "continue";
                if (!candidates.some(function (c) { return c.provider === match.provider && c.model === match.model; })) {
    ...
    if (!resolved) {
        ...
        if (fuzzy_1.selection || fuzzy_1.error)
            return fuzzy_1;
        return {
            error: "Unrecognized model \"".concat(rawTrimmed, "\". Use /models to list providers, or /models <provider> to list models."),
        };
    }
    ...
    if (fuzzy.selection || fuzzy.error)
        return fuzzy;
    return {
        error: "Model \"".concat(resolved.ref.provider, "/").concat(resolved.ref.model, "\" is not allowed. Use /models to list providers, or /models <provider> to list models."),
    };
}
function resolveContextTokens(params) {
    var _a, _b, _c;
    return ((_c = (_b = (_a = params.agentCfg) === null || _a === void 0 ? void 0 : _a.contextTokens) !== null && _b !== void 0 ? _b : (0, context_js_1.lookupContextTokens)(params.model)) !== null && _c !== void 0 ? _c : defaults_js_1.DEFAULT_CONTEXT_TOKENS);
}
