...
exports.DEFAULT_HEARTBEAT_ACK_MAX_CHARS = exports.DEFAULT_HEARTBEAT_EVERY = exports.HEARTBEAT_PROMPT = void 0;
...
exports.HEARTBEAT_PROMPT = "Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.";
...
function stripHeartbeatToken(raw, opts) {
    ...
    var maxAckChars = Math.max(0, typeof parsedAckChars === "number" && Number.isFinite(parsedAckChars)
        ? parsedAckChars
    ...
    var hasToken = trimmed.includes(tokens_js_1.HEARTBEAT_TOKEN) || trimmedNormalized.includes(tokens_js_1.HEARTBEAT_TOKEN);
    if (!hasToken) {
        return { shouldSkip: false, text: trimmed, didStrip: false };
    }
    var strippedOriginal = stripTokenAtEdges(trimmed);
    var strippedNormalized = stripTokenAtEdges(trimmedNormalized);
    var picked = strippedOriginal.didStrip && strippedOriginal.text ? strippedOriginal : strippedNormalized;
    if (!picked.didStrip) {
        return { shouldSkip: false, text: trimmed, didStrip: false };
    }
    if (!picked.text) {
