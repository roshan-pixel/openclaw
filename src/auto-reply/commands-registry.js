...
function getTextAliasMap() {
    ...
    for (var _i = 0, commands_1 = commands; _i < commands_1.length; _i++) {
        ...
        var canonical = ((_a = command.textAliases[0]) === null || _a === void 0 ? void 0 : _a.trim()) || "/".concat(command.key);
...
function buildSkillCommandDefinitions(skillCommands) {
    if (!skillCommands || skillCommands.length === 0)
        return [];
    return skillCommands.map(function (spec) { return ({
...
function listChatCommands(params) {
    var _a;
    var commands = (0, commands_registry_data_js_1.getChatCommands)();
    if (!((_a = params === null || params === void 0 ? void 0 : params.skillCommands) === null || _a === void 0 ? void 0 : _a.length))
        return __spreadArray([], commands, true);
    return __spreadArray(__spreadArray([], commands, true), buildSkillCommandDefinitions(params.skillCommands), true);
}
...
function listChatCommandsForConfig(cfg, params) {
    var _a;
    var base = (0, commands_registry_data_js_1.getChatCommands)().filter(function (command) { return isCommandEnabled(cfg, command.key); });
    if (!((_a = params === null || params === void 0 ? void 0 : params.skillCommands) === null || _a === void 0 ? void 0 : _a.length))
        return base;
    return __spreadArray(__spreadArray([], base, true), buildSkillCommandDefinitions(params.skillCommands), true);
}
...
function resolveNativeName(command, provider) {
    ...
    if (!command.nativeName)
        return undefined;
    if (provider) {
        var override = (_a = NATIVE_NAME_OVERRIDES[provider]) === null || _a === void 0 ? void 0 : _a[command.key];
        if (override)
            return override;
    }
...
function listNativeCommandSpecs(params) {
    return listChatCommands({ skillCommands: params === null || params === void 0 ? void 0 : params.skillCommands })
        .filter(function (command) { return command.scope !== "text" && command.nativeName; })
        .map(function (command) {
        var _a;
        return ({
            name: (_a = resolveNativeName(command, params === null || params === void 0 ? void 0 : params.provider)) !== null && _a !== void 0 ? _a : command.key,
...
function findCommandByNativeName(name, provider) {
    ...
    return (0, commands_registry_data_js_1.getChatCommands)().find(function (command) {
        ...
        return command.scope !== "text" &&
            ((_a = resolveNativeName(command, provider)) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === normalized;
    });
}
...
function resolveDefaultCommandContext(cfg) {
    ...
    return {
        provider: (_a = resolved.provider) !== null && _a !== void 0 ? _a : defaults_js_1.DEFAULT_PROVIDER,
        model: (_b = resolved.model) !== null && _b !== void 0 ? _b : defaults_js_1.DEFAULT_MODEL,
    };
}
function resolveCommandArgChoices(params) {
    var command = params.command, arg = params.arg, cfg = params.cfg;
    if (!arg.choices)
        return [];
...
function resolveCommandArgMenu(params) {
    var _a;
    var command = params.command, args = params.args, cfg = params.cfg;
    if (!command.args || !command.argsMenu)
        return null;
    if (command.argsParsing === "none")
        return null;
    var argSpec = command.argsMenu;
    var argName = argSpec === "auto"
        ? (_a = command.args.find(function (arg) { return resolveCommandArgChoices({ command: command, arg: arg, cfg: cfg }).length > 0; })) === null || _a === void 0 ? void 0 : _a.name
        : argSpec.arg;
    if (!argName)
        return null;
    ...
    if ((args === null || args === void 0 ? void 0 : args.raw) && !args.values)
        return null;
    var arg = command.args.find(function (entry) { return entry.name === argName; });
    if (!arg)
        return null;
...
function normalizeCommandBody(raw, options) {
    ...
    var normalized = colonMatch
        ? (function () {
            var command = colonMatch[1], rest = colonMatch[2];
            var normalizedRest = rest.trimStart();
            return normalizedRest ? "/".concat(command, " ").concat(normalizedRest) : "/".concat(command);
        })()
        : singleLine;
    var normalizedBotUsername = (_a = options === null || options === void 0 ? void 0 : options.botUsername) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase();
    var mentionMatch = normalizedBotUsername
        ? normalized.match(/^\/([^\s@]+)@([^\s]+)(.*)$/)
        : null;
    var commandBody = mentionMatch && mentionMatch[2].toLowerCase() === normalizedBotUsername
        ? "/".concat(mentionMatch[1]).concat((_b = mentionMatch[3]) !== null && _b !== void 0 ? _b : "")
        : normalized;
    ...
    if (!tokenMatch)
        return commandBody;
    var token = tokenMatch[1], rest = tokenMatch[2];
    ...
    return normalizedRest ? "".concat(tokenSpec.canonical, " ").concat(normalizedRest) : tokenSpec.canonical;
}
...
function resolveTextCommand(raw, cfg) {
    ...
    if (!spec)
        return null;
    var command = (0, commands_registry_data_js_1.getChatCommands)().find(function (entry) { return entry.key === spec.key; });
    if (!command)
        return null;
    if (!spec.acceptsArgs)
