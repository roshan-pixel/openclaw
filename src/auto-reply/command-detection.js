...
function hasControlCommand(text, cfg, options) {
    ...
    var commands = cfg ? (0, commands_registry_js_1.listChatCommandsForConfig)(cfg) : (0, commands_registry_js_1.listChatCommands)();
...
function isControlCommandMessage(text, cfg, options) {
    ...
    if (hasControlCommand(trimmed, cfg, options))
        return true;
    var normalized = (0, commands_registry_js_1.normalizeCommandBody)(trimmed, options).trim().toLowerCase();
