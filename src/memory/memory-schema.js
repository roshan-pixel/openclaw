...
function ensureMemoryIndexSchema(params) {
    ...
    params.db.exec("\n    CREATE TABLE IF NOT EXISTS ".concat(params.embeddingCacheTable, " (\n      provider TEXT NOT NULL,\n      model TEXT NOT NULL,\n      provider_key TEXT NOT NULL,\n      hash TEXT NOT NULL,\n      embedding TEXT NOT NULL,\n      dims INTEGER,\n      updated_at INTEGER NOT NULL,\n      PRIMARY KEY (provider, model, provider_key, hash)\n    );\n  "));
    params.db.exec("CREATE INDEX IF NOT EXISTS idx_embedding_cache_updated_at ON ".concat(params.embeddingCacheTable, "(updated_at);"));
    var ftsAvailable = false;
    var ftsError;
    if (params.ftsEnabled) {
        try {
            params.db.exec("CREATE VIRTUAL TABLE IF NOT EXISTS ".concat(params.ftsTable, " USING fts5(\n") +
                "  text,\n" +
                "  id UNINDEXED,\n" +
                "  path UNINDEXED,\n" +
                "  source UNINDEXED,\n" +
...
function ensureColumn(db, table, column, definition) {
    ...
    if (rows.some(function (row) { return row.name === column; }))
        return;
