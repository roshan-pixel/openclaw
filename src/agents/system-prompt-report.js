...
function parseSkillBlocks(skillsPrompt) {
    ...
    if (!prompt)
        return [];
    var blocks = Array.from(prompt.matchAll(/<skill>[\s\S]*?<\/skill>/gi)).map(function (match) { var _a; return (_a = match[0]) !== null && _a !== void 0 ? _a : ""; });
    return blocks
        .map(function (block) {
        var _a, _b;
        var name = ((_b = (_a = block.match(/<name>\s*([^<]+?)\s*<\/name>/i)) === null || _a === void 0 ? void 0 : _a[1]) === null || _b === void 0 ? void 0 : _b.trim()) || "(unknown)";
        return { name: name, blockChars: block.length };
    })
        .filter(function (b) { return b.blockChars > 0; });
}
function buildInjectedWorkspaceFiles(params) {
    var injectedByName = new Map(params.injectedFiles.map(function (f) { return [f.path, f.content]; }));
    return params.bootstrapFiles.map(function (file) {
        var _a;
        var rawChars = file.missing ? 0 : ((_a = file.content) !== null && _a !== void 0 ? _a : "").trimEnd().length;
...
function buildToolsEntries(tools) {
    return tools.map(function (tool) {
        var _a, _b;
        var name = tool.name;
        var summary = ((_a = tool.description) === null || _a === void 0 ? void 0 : _a.trim()) || ((_b = tool.label) === null || _b === void 0 ? void 0 : _b.trim()) || "";
        ...
        return { name: name, summaryChars: summaryChars, schemaChars: schemaChars, propertiesCount: propertiesCount };
    });
}
...
function buildSystemPromptReport(params) {
    ...
    var projectContext = extractBetween(systemPrompt, "\n# Project Context\n", "\n## Silent Replies\n");
    ...
    var toolsSchemaChars = toolsEntries.reduce(function (sum, t) { var _a; return sum + ((_a = t.schemaChars) !== null && _a !== void 0 ? _a : 0); }, 0);
