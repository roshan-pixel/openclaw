            ...
            switch (_d.label) {
                case 0:
                    ...
                    if (resumeArgs.length === 0)
                        return [2 /*return*/];
                    if (!resumeArgs.some(function (arg) { return arg.includes("{sessionId}"); }))
                        return [2 /*return*/];
                    commandToken = node_path_1.default.basename((_c = backend.command) !== null && _c !== void 0 ? _c : "").trim();
                    if (!commandToken)
                        return [2 /*return*/];
                    resumeTokens = resumeArgs.map(function (arg) { return arg.replaceAll("{sessionId}", sessionId); });
                    pattern = __spreadArray([commandToken], resumeTokens, true).filter(Boolean)
...
function buildSessionMatchers(backend) {
    var _a, _b, _c, _d;
    var commandToken = node_path_1.default.basename((_a = backend.command) !== null && _a !== void 0 ? _a : "").trim();
    if (!commandToken)
        return [];
    var matchers = [];
    ...
    if (sessionArgs.some(function (arg) { return arg.includes("{sessionId}"); })) {
...
function tokenToRegex(token) {
    ...
    var parts = token.split("{sessionId}").map(function (part) { return escapeRegex(part); });
    return parts.join("\\S+");
}
/**
            ...
            switch (_e.label) {
                ...
                case 2:
                    ...
                    _loop_1 = function (line) {
                        ...
                        if (!stat.includes("T"))
                            return "continue";
                        if (!matchers.some(function (matcher) { return matcher.test(command); }))
                            return "continue";
                        suspended.push(pid);
                    };
                    ...
                    if (!(suspended.length > threshold)) return [3 /*break*/, 4];
                    // Verified locally: stopped (T) processes ignore SIGTERM, so use SIGKILL.
                    return [4 /*yield*/, (0, exec_js_1.runExec)("kill", __spreadArray(["-9"], suspended.map(function (pid) { return String(pid); }), true))];
                case 3:
                    // Verified locally: stopped (T) processes ignore SIGTERM, so use SIGKILL.
                    _e.sent();
                    _e.label = 4;
...
function enqueueCliRun(key, task) {
    ...
    var chained = prior.catch(function () { return undefined; }).then(task);
...
function buildModelAliasLines(cfg) {
    var _a, _b, _c, _d;
    var models = (_c = (_b = (_a = cfg === null || cfg === void 0 ? void 0 : cfg.agents) === null || _a === void 0 ? void 0 : _a.defaults) === null || _b === void 0 ? void 0 : _b.models) !== null && _c !== void 0 ? _c : {};
    var entries = [];
    for (var _i = 0, _e = Object.entries(models); _i < _e.length; _i++) {
        var _f = _e[_i], keyRaw = _f[0], entryRaw = _f[1];
    ...
    return entries
        ...
        .map(function (entry) { return "- ".concat(entry.alias, ": ").concat(entry.model); });
}
function buildSystemPrompt(params) {
    ...
    return (0, system_prompt_js_1.buildAgentSystemPrompt)({
        ...
        toolNames: params.tools.map(function (tool) { return tool.name; }),
...
function toUsage(raw) {
    ...
    var cacheRead = (_d = (_c = pick("cache_read_input_tokens")) !== null && _c !== void 0 ? _c : pick("cached_input_tokens")) !== null && _d !== void 0 ? _d : pick("cacheRead");
    var cacheWrite = (_e = pick("cache_write_input_tokens")) !== null && _e !== void 0 ? _e : pick("cacheWrite");
...
function collectText(value) {
    ...
    if (Array.isArray(value))
        return value.map(function (entry) { return collectText(entry); }).join("");
    if (!isRecord(value))
        return "";
    ...
    if (Array.isArray(value.content))
        return value.content.map(function (entry) { return collectText(entry); }).join("");
...
function pickSessionId(parsed, backend) {
    var _a;
    var fields = (_a = backend.sessionIdFields) !== null && _a !== void 0 ? _a : [
        "session_id",
        "sessionId",
        "conversation_id",
        "conversationId",
...
function parseCliJson(raw, backend) {
    ...
    var text = collectText(parsed.message) ||
...
function writeCliImages(images) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, promises_1.default.mkdtemp(node_path_1.default.join(node_os_1.default.tmpdir(), "openclaw-cli-images-"))];
                case 1:
                    tempDir = _a.sent();
                    paths = [];
                    i = 0;
