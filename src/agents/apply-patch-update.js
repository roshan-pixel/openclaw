...
function applyUpdateHunk(filePath, chunks) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, promises_1.default.readFile(filePath, "utf8").catch(function (err) {
...
function computeReplacements(originalLines, filePath, chunks) {
    ...
    for (var _i = 0, chunks_1 = chunks; _i < chunks_1.length; _i++) {
        ...
        if (chunk.changeContext) {
            ...
            if (ctxIndex === null) {
                throw new Error("Failed to find context '".concat(chunk.changeContext, "' in ").concat(filePath));
            }
            lineIndex = ctxIndex + 1;
        }
        if (chunk.oldLines.length === 0) {
            var insertionIndex = originalLines.length > 0 && originalLines[originalLines.length - 1] === ""
        ...
        if (found === null) {
            throw new Error("Failed to find expected lines in ".concat(filePath, ":\n").concat(chunk.oldLines.join("\n")));
        }
        replacements.push([found, pattern.length, newSlice]);
        lineIndex = found + pattern.length;
    }
    replacements.sort(function (a, b) { return a[0] - b[0]; });
    return replacements;
}
function applyReplacements(lines, replacements) {
    var result = __spreadArray([], lines, true);
    for (var _i = 0, _a = __spreadArray([], replacements, true).reverse(); _i < _a.length; _i++) {
        var _b = _a[_i], startIndex = _b[0], oldLen = _b[1], newLines = _b[2];
...
function seekSequence(lines, pattern, start, eof) {
    ...
    for (var i = searchStart; i <= maxStart; i += 1) {
        if (linesMatch(lines, pattern, i, function (value) { return value; }))
            return i;
    }
    for (var i = searchStart; i <= maxStart; i += 1) {
        if (linesMatch(lines, pattern, i, function (value) { return value.trimEnd(); }))
            return i;
    }
    for (var i = searchStart; i <= maxStart; i += 1) {
        if (linesMatch(lines, pattern, i, function (value) { return value.trim(); }))
            return i;
    }
    for (var i = searchStart; i <= maxStart; i += 1) {
        if (linesMatch(lines, pattern, i, function (value) { return normalizePunctuation(value.trim()); })) {
            return i;
        }
    }
    return null;
