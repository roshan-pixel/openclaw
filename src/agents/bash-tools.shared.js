...
function buildSandboxEnv(params) {
    ...
    for (var _i = 0, _c = Object.entries((_a = params.sandboxEnv) !== null && _a !== void 0 ? _a : {}); _i < _c.length; _i++) {
        var _d = _c[_i], key = _d[0], value = _d[1];
        env[key] = value;
    }
    for (var _e = 0, _f = Object.entries((_b = params.paramsEnv) !== null && _b !== void 0 ? _b : {}); _e < _f.length; _e++) {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 3:
                    ...
                    return [2 /*return*/, { hostWorkdir: resolved.resolved, containerWorkdir: containerWorkdir }];
                case 4:
                    _a = _b.sent();
                    params.warnings.push("Warning: workdir \"".concat(params.workdir, "\" is unavailable; using \"").concat(fallback, "\"."));
                    return [2 /*return*/, {
...
function killSession(session) {
    var _a, _b;
    var pid = (_a = session.pid) !== null && _a !== void 0 ? _a : (_b = session.child) === null || _b === void 0 ? void 0 : _b.pid;
    if (pid) {
...
function resolveWorkdir(workdir, warnings) {
    ...
    warnings.push("Warning: workdir \"".concat(workdir, "\" is unavailable; using \"").concat(fallback, "\"."));
    return fallback;
}
function safeCwd() {
    try {
...
function chunkString(input, limit) {
    if (limit === void 0) { limit = CHUNK_LIMIT; }
    var chunks = [];
...
function truncateMiddle(str, max) {
    ...
    return "".concat((0, utils_js_1.sliceUtf16Safe)(str, 0, half), "...").concat((0, utils_js_1.sliceUtf16Safe)(str, -half));
}
function sliceLogLines(text, offset, limit) {
    ...
    var start = typeof offset === "number" && Number.isFinite(offset) ? Math.max(0, Math.floor(offset)) : 0;
    if (limit !== undefined && offset === undefined) {
        var tailCount = Math.max(0, Math.floor(limit));
        start = Math.max(totalLines - tailCount, 0);
    }
    var end = typeof limit === "number" && Number.isFinite(limit)
        ? start + Math.max(0, Math.floor(limit))
        : undefined;
    return { slice: lines.slice(start, end).join("\n"), totalLines: totalLines, totalChars: totalChars };
}
function deriveSessionName(command) {
    ...
    if (tokens.length === 0)
        return undefined;
    var verb = tokens[0];
    var target = tokens.slice(1).find(function (t) { return !t.startsWith("-"); });
    if (!target)
        target = tokens[1];
    if (!target)
        return verb;
...
function tokenizeCommand(command) {
    var _a;
    var matches = (_a = command.match(/(?:[^\s"']+|"(?:\\.|[^"])*"|'(?:\\.|[^'])*')+/g)) !== null && _a !== void 0 ? _a : [];
    return matches.map(function (token) { return stripQuotes(token); }).filter(Boolean);
}
