...
function subscribeEmbeddedPiSession(params) {
    ...
    var state = {
        ...
        blockState: { thinking: false, final: false, inlineCode: (0, code_spans_js_1.createInlineCodeState)() },
    ...
    var replyDirectiveAccumulator = (0, streaming_directives_js_1.createStreamingDirectiveAccumulator)();
    ...
    var finalizeAssistantTexts = function (args) {
        var text = args.text, addedDuringMessage = args.addedDuringMessage, chunkerHasBuffered = args.chunkerHasBuffered;
    ...
    var blockChunker = blockChunking ? new pi_embedded_block_chunker_js_1.EmbeddedBlockChunker(blockChunking) : null;
    ...
    var emitToolSummary = function (toolName, meta) {
        ...
        var _a = (0, reply_directives_js_1.parseReplyDirectives)(agg), cleanedText = _a.text, mediaUrls = _a.mediaUrls;
    ...
    var emitToolOutput = function (toolName, meta, output) {
        ...
        var _a = (0, reply_directives_js_1.parseReplyDirectives)(message), cleanedText = _a.text, mediaUrls = _a.mediaUrls;
        if (!cleanedText && (!mediaUrls || mediaUrls.length === 0))
            return;
        try {
            void params.onToolResult({
    ...
    var stripBlockTags = function (text, state) {
        ...
        if (!text)
            return text;
        var inlineStateStart = (_a = state.inlineCode) !== null && _a !== void 0 ? _a : (0, code_spans_js_1.createInlineCodeState)();
    ...
    var emitBlockChunk = function (text) {
        ...
        if ((0, pi_embedded_helpers_js_1.isMessagingToolDuplicateNormalized)(normalizedChunk, messagingToolSentTextsNormalized)) {
            log.debug("Skipping block reply - already sent via messaging tool: ".concat(chunk.slice(0, 50), "..."));
            return;
        }
    ...
    var unsubscribe = params.session.subscribe((0, pi_embedded_subscribe_handlers_js_1.createEmbeddedPiSessionEventHandler)(ctx));
    return {
        assistantTexts: assistantTexts,
        toolMetas: toolMetas,
        unsubscribe: unsubscribe,
        isCompacting: function () { return state.compactionInFlight || state.pendingCompactionRetry > 0; },
        getMessagingToolSentTexts: function () { return messagingToolSentTexts.slice(); },
        getMessagingToolSentTargets: function () { return messagingToolSentTargets.slice(); },
        // Returns true if any messaging tool successfully sent a message.
        // Used to suppress agent's confirmation text (e.g., "Respondi no Telegram!")
        // which is generated AFTER the tool sends the actual answer.
        didSendViaMessagingTool: function () { return messagingToolSentTexts.length > 0; },
        getLastToolError: function () { return (state.lastToolError ? __assign({}, state.lastToolError) : undefined); },
        waitForCompactionRetry: function () {
            var _a;
            if (state.compactionInFlight || state.pendingCompactionRetry > 0) {
                ensureCompactionPromise();
                return (_a = state.compactionRetryPromise) !== null && _a !== void 0 ? _a : Promise.resolve();
            }
            return new Promise(function (resolve) {
                queueMicrotask(function () {
                    ...
                    if (state.compactionInFlight || state.pendingCompactionRetry > 0) {
                        ensureCompactionPromise();
                        void ((_a = state.compactionRetryPromise) !== null && _a !== void 0 ? _a : Promise.resolve()).then(resolve);
                    }
                    else {
                        resolve();
                    }
