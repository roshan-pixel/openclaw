            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, applyPatch(input, {
                                cwd: cwd,
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    ...
                    if (!(hunk.kind === "add")) return [3 /*break*/, 5];
                ...
                case 3:
                    _c.sent();
                    return [4 /*yield*/, promises_1.default.writeFile(target_1.resolved, hunk.contents, "utf8")];
                case 4:
                    _c.sent();
                    recordSummary(summary, seen, "added", target_1.display);
                    return [3 /*break*/, 17];
                case 5:
                    if (!(hunk.kind === "delete")) return [3 /*break*/, 8];
                    return [4 /*yield*/, resolvePatchPath(hunk.path, options)];
                case 6:
                    target_2 = _c.sent();
                ...
                case 7:
                    ...
                    return [3 /*break*/, 17];
                case 8: return [4 /*yield*/, resolvePatchPath(hunk.path, options)];
                case 9:
                    target = _c.sent();
                    return [4 /*yield*/, (0, apply_patch_update_js_1.applyUpdateHunk)(target.resolved, hunk.chunks)];
                case 10:
                    applied = _c.sent();
                    if (!hunk.movePath) return [3 /*break*/, 15];
                ...
                case 14:
                    ...
                    return [3 /*break*/, 17];
                case 15: return [4 /*yield*/, promises_1.default.writeFile(target.resolved, applied, "utf8")];
                case 16:
                    _c.sent();
...
function resolvePatchPath(filePath, options) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!options.sandboxRoot) return [3 /*break*/, 2];
...
function parsePatchText(input) {
    ...
    while (remaining.length > 0) {
        var _a = parseOneHunk(remaining, lineNumber), hunk = _a.hunk, consumed = _a.consumed;
        hunks.push(hunk);
        lineNumber += consumed;
...
function parseOneHunk(lines, lineNumber) {
    ...
    if (firstLine.startsWith(UPDATE_FILE_MARKER)) {
        ...
        if (moveCandidate === null || moveCandidate === void 0 ? void 0 : moveCandidate.startsWith(MOVE_TO_MARKER)) {
        ...
        while (remaining.length > 0) {
            ...
            var _c = parseUpdateFileChunk(remaining, lineNumber + consumed, chunks.length === 0), chunk = _c.chunk, chunkLines = _c.consumed;
            chunks.push(chunk);
            remaining = remaining.slice(chunkLines);
            consumed += chunkLines;
        }
        if (chunks.length === 0) {
            throw new Error("Invalid patch hunk at line ".concat(lineNumber, ": Update file hunk for path '").concat(targetPath, "' is empty"));
        }
        return {
            hunk: {
                kind: "update",
    ...
    throw new Error("Invalid patch hunk at line ".concat(lineNumber, ": '").concat(lines[0], "' is not a valid hunk header. Valid hunk headers: '*** Add File: {path}', '*** Delete File: {path}', '*** Update File: {path}'"));
}
function parseUpdateFileChunk(lines, lineNumber, allowMissingContext) {
    if (lines.length === 0) {
        throw new Error("Invalid patch hunk at line ".concat(lineNumber, ": Update hunk does not contain any lines"));
    }
    var changeContext;
    var startIndex = 0;
    if (lines[0] === EMPTY_CHANGE_CONTEXT_MARKER) {
        ...
        throw new Error("Invalid patch hunk at line ".concat(lineNumber, ": Expected update hunk to start with a @@ context marker, got: '").concat(lines[0], "'"));
    }
    if (startIndex >= lines.length) {
        throw new Error("Invalid patch hunk at line ".concat(lineNumber + 1, ": Update hunk does not contain any lines"));
    }
    var chunk = {
    ...
    for (var _i = 0, _a = lines.slice(startIndex); _i < _a.length; _i++) {
        ...
        if (parsedLines === 0) {
            throw new Error("Invalid patch hunk at line ".concat(lineNumber + 1, ": Unexpected line found in update hunk: '").concat(line, "'. Every line should start with ' ' (context line), '+' (added line), or '-' (removed line)"));
        }
        break;
    }
