...
function loadSubagentRegistryFromDisk() {
    ...
    for (var _i = 0, _c = Object.entries(runsRaw); _i < _c.length; _i++) {
        var _d = _c[_i], runId = _d[0], entry = _d[1];
        if (!entry || typeof entry !== "object")
            continue;
        var typed = entry;
        if (!typed.runId || typeof typed.runId !== "string")
            continue;
        var legacyCompletedAt = isLegacy && typeof typed.announceCompletedAt === "number"
            ? typed.announceCompletedAt
            : undefined;
        var cleanupCompletedAt = typeof typed.cleanupCompletedAt === "number" ? typed.cleanupCompletedAt : legacyCompletedAt;
        var cleanupHandled = typeof typed.cleanupHandled === "boolean"
            ? typed.cleanupHandled
            : isLegacy
                ? Boolean((_a = typed.announceHandled) !== null && _a !== void 0 ? _a : cleanupCompletedAt)
                : undefined;
        var requesterOrigin = (0, delivery_context_js_1.normalizeDeliveryContext)((_b = typed.requesterOrigin) !== null && _b !== void 0 ? _b : {
        ...
        var _announceCompletedAt = typed.announceCompletedAt, _announceHandled = typed.announceHandled, _channel = typed.requesterChannel, _accountId = typed.requesterAccountId, rest = __rest(typed, ["announceCompletedAt", "announceHandled", "requesterChannel", "requesterAccountId"]);
        out.set(runId, __assign(__assign({}, rest), { requesterOrigin: requesterOrigin, cleanupCompletedAt: cleanupCompletedAt, cleanupHandled: cleanupHandled }));
        if (isLegacy)
            migrated = true;
    }
    if (migrated) {
