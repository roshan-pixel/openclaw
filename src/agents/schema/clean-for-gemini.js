...
function stripNullVariants(variants) {
    ...
    var nonNull = variants.filter(function (variant) { return !isNullSchema(variant); });
    return {
        variants: nonNull,
...
function extendSchemaDefs(defs, schema) {
    var defsEntry = schema.$defs && typeof schema.$defs === "object" && !Array.isArray(schema.$defs)
        ? schema.$defs
        : undefined;
    var legacyDefsEntry = schema.definitions &&
...
function cleanSchemaForGeminiWithDefs(schema, defs, refStack) {
    ...
    if (Array.isArray(schema)) {
        return schema.map(function (item) { return cleanSchemaForGeminiWithDefs(item, defs, refStack); });
    }
    var obj = schema;
    ...
    var cleanedOneOf = hasOneOf
        ...
        : undefined;
    if (hasAnyOf) {
        var _d = stripNullVariants(cleanedAnyOf !== null && cleanedAnyOf !== void 0 ? cleanedAnyOf : []), nonNullVariants = _d.variants, stripped = _d.stripped;
        if (stripped)
    ...
    if (hasOneOf) {
        var _j = stripNullVariants(cleanedOneOf !== null && cleanedOneOf !== void 0 ? cleanedOneOf : []), nonNullVariants = _j.variants, stripped = _j.stripped;
        if (stripped)
    ...
    for (var _p = 0, _q = Object.entries(obj); _p < _q.length; _p++) {
        ...
        if (key === "type" &&
            Array.isArray(value) &&
            value.every(function (entry) { return typeof entry === "string"; })) {
            var types = value.filter(function (entry) { return entry !== "null"; });
            cleaned.type = types.length === 1 ? types[0] : types;
            continue;
        }
        if (key === "properties" && value && typeof value === "object") {
            var props = value;
            cleaned[key] = Object.fromEntries(Object.entries(props).map(function (_a) {
                var k = _a[0], v = _a[1];
                return [
                    k,
            ...
            cleaned[key] =
                cleanedAnyOf !== null && cleanedAnyOf !== void 0 ? cleanedAnyOf : value.map(function (variant) { return cleanSchemaForGeminiWithDefs(variant, nextDefs, refStack); });
        }
        else if (key === "oneOf" && Array.isArray(value)) {
            cleaned[key] =
                cleanedOneOf !== null && cleanedOneOf !== void 0 ? cleanedOneOf : value.map(function (variant) { return cleanSchemaForGeminiWithDefs(variant, nextDefs, refStack); });
        }
