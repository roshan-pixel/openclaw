...
function handleToolExecutionStart(ctx, evt) {
    return __awaiter(this, void 0, void 0, function () {
        var rawToolName, toolName, toolCallId, args, record, filePath, argsPreview, meta, shouldEmitToolEvents, argsRecord, isMessagingSend, sendTarget, text;
        var _a, _b, _c;
        return __generator(this, function (_d) {
            ...
            if (toolName === "read") {
                ...
                if (!filePath) {
                    ...
                    ctx.log.warn("read tool called without path: toolCallId=".concat(toolCallId, " argsType=").concat(typeof args).concat(argsPreview ? " argsPreview=".concat(argsPreview) : ""));
                }
            }
            meta = extendExecMeta(toolName, args, (0, pi_embedded_utils_js_1.inferToolMetaFromArgs)(toolName, args));
            ctx.state.toolMetaById.set(toolCallId, meta);
            ctx.log.debug("embedded run tool start: runId=".concat(ctx.params.runId, " tool=").concat(toolName, " toolCallId=").concat(toolCallId));
            ...
            void ((_b = (_a = ctx.params).onAgentEvent) === null || _b === void 0 ? void 0 : _b.call(_a, {
                stream: "tool",
            ...
            if ((0, pi_embedded_messaging_js_1.isMessagingTool)(toolName)) {
                ...
                isMessagingSend = (0, pi_embedded_messaging_js_1.isMessagingToolSendAction)(toolName, argsRecord);
                if (isMessagingSend) {
                    sendTarget = (0, pi_embedded_subscribe_tools_js_1.extractMessagingToolSend)(toolName, argsRecord);
                    if (sendTarget) {
                    ...
                    if (text && typeof text === "string") {
                        ...
                        ctx.log.debug("Tracking pending messaging text: tool=".concat(toolName, " len=").concat(text.length));
                    }
                }
            }
            return [2 /*return*/];
...
function handleToolExecutionEnd(ctx, evt) {
    ...
    if (isToolError) {
        var errorMessage = (0, pi_embedded_subscribe_tools_js_1.extractToolErrorMessage)(sanitizedResult);
    ...
    if (pendingText) {
        ...
        if (!isToolError) {
            ...
            ctx.state.messagingToolSentTextsNormalized.push((0, pi_embedded_helpers_js_1.normalizeTextForComparison)(pendingText));
            ctx.log.debug("Committed messaging text: tool=".concat(toolName, " len=").concat(pendingText.length));
    ...
    ctx.log.debug("embedded run tool end: runId=".concat(ctx.params.runId, " tool=").concat(toolName, " toolCallId=").concat(toolCallId));
