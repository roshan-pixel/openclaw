...
function isAnthropicApi(modelApi, provider) {
    if (modelApi === "anthropic-messages")
        return true;
    var normalized = (0, model_selection_js_1.normalizeProviderId)(provider !== null && provider !== void 0 ? provider : "");
    // MiniMax now uses openai-completions API, not anthropic-messages
    return normalized === "anthropic";
}
function isMistralModel(params) {
    var _a, _b;
    var provider = (0, model_selection_js_1.normalizeProviderId)((_a = params.provider) !== null && _a !== void 0 ? _a : "");
    if (provider === "mistral")
        return true;
    var modelId = ((_b = params.modelId) !== null && _b !== void 0 ? _b : "").toLowerCase();
    if (!modelId)
        return false;
    return MISTRAL_MODEL_HINTS.some(function (hint) { return modelId.includes(hint); });
}
function resolveTranscriptPolicy(params) {
    ...
    var isOpenRouterGemini = (provider === "openrouter" || provider === "opencode") &&
