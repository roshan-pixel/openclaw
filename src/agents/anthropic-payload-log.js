...
function resolvePayloadLogConfig(env) {
    var _a, _b;
    var enabled = (_a = (0, boolean_js_1.parseBooleanValue)(env.OPENCLAW_ANTHROPIC_PAYLOAD_LOG)) !== null && _a !== void 0 ? _a : false;
    var fileOverride = (_b = env.OPENCLAW_ANTHROPIC_PAYLOAD_LOG_FILE) === null || _b === void 0 ? void 0 : _b.trim();
    var filePath = fileOverride
        ? (0, utils_js_1.resolveUserPath)(fileOverride)
        : node_path_1.default.join((0, paths_js_1.resolveStateDir)(env), "logs", "anthropic-payload.jsonl");
...
function findLastAssistantUsage(messages) {
    for (var i = messages.length - 1; i >= 0; i -= 1) {
        var msg = messages[i];
        if ((msg === null || msg === void 0 ? void 0 : msg.role) === "assistant" && msg.usage && typeof msg.usage === "object") {
            return msg.usage;
        }
    }
    return null;
...
function createAnthropicPayloadLogger(params) {
    ...
    var wrapStreamFn = function (streamFn) {
        var wrapped = function (model, context, options) {
            ...
            var nextOnPayload = function (payload) {
                var _a;
                record(__assign(__assign({}, base), { ts: new Date().toISOString(), stage: "request", payload: payload, payloadDigest: digest(payload) }));
                (_a = options === null || options === void 0 ? void 0 : options.onPayload) === null || _a === void 0 ? void 0 : _a.call(options, payload);
            };
            return streamFn(model, context, __assign(__assign({}, options), { onPayload: nextOnPayload }));
        };
        return wrapped;
    };
    var recordUsage = function (messages, error) {
        ...
        if (!usage) {
            if (errorMessage) {
                record(__assign(__assign({}, base), { ts: new Date().toISOString(), stage: "usage", error: errorMessage }));
            }
            return;
        }
        record(__assign(__assign({}, base), { ts: new Date().toISOString(), stage: "usage", usage: usage, error: errorMessage }));
