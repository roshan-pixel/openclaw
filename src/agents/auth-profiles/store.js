        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, proper_lockfile_1.default.lock(authPath, constants_js_1.AUTH_STORE_LOCK_OPTIONS)];
                case 2:
                    release = _c.sent();
...
function coerceLegacyStore(raw) {
    ...
    for (var _i = 0, _b = Object.entries(record); _i < _b.length; _i++) {
        ...
        entries[key] = __assign(__assign({}, typed), { provider: String((_a = typed.provider) !== null && _a !== void 0 ? _a : key) });
    }
...
function coerceAuthStore(raw) {
    ...
    var order = record.order && typeof record.order === "object"
        ? Object.entries(record.order).reduce(function (acc, _a) {
            var provider = _a[0], value = _a[1];
        ...
        : undefined;
    return {
        version: Number((_a = record.version) !== null && _a !== void 0 ? _a : constants_js_1.AUTH_STORE_VERSION),
        profiles: normalized,
        order: order,
...
function mergeAuthProfileStores(base, override) {
    ...
    return {
        version: Math.max(base.version, (_a = override.version) !== null && _a !== void 0 ? _a : base.version),
...
function mergeOAuthFileIntoStore(store) {
    ...
    for (var _i = 0, _a = Object.entries(oauthEntries); _i < _a.length; _i++) {
        var _b = _a[_i], provider = _b[0], creds = _b[1];
...
function loadAuthProfileStore() {
    ...
    if (legacy) {
        ...
        for (var _i = 0, _d = Object.entries(legacy); _i < _d.length; _i++) {
            var _e = _d[_i], provider = _e[0], cred = _e[1];
            var profileId = "".concat(provider, ":default");
            if (cred.type === "api_key") {
                store_1.profiles[profileId] = __assign({ type: "api_key", provider: String((_a = cred.provider) !== null && _a !== void 0 ? _a : provider), key: cred.key }, (cred.email ? { email: cred.email } : {}));
            }
            else if (cred.type === "token") {
                store_1.profiles[profileId] = __assign(__assign({ type: "token", provider: String((_b = cred.provider) !== null && _b !== void 0 ? _b : provider), token: cred.token }, (typeof cred.expires === "number" ? { expires: cred.expires } : {})), (cred.email ? { email: cred.email } : {}));
            }
            else {
                store_1.profiles[profileId] = __assign(__assign(__assign(__assign({ type: "oauth", provider: String((_c = cred.provider) !== null && _c !== void 0 ? _c : provider), access: cred.access, refresh: cred.refresh, expires: cred.expires }, (cred.enterpriseUrl ? { enterpriseUrl: cred.enterpriseUrl } : {})), (cred.projectId ? { projectId: cred.projectId } : {})), (cred.accountId ? { accountId: cred.accountId } : {})), (cred.email ? { email: cred.email } : {}));
            }
        }
...
function loadAuthProfileStoreForAgent(agentDir, _options) {
    ...
    var legacyRaw = (0, json_file_js_1.loadJsonFile)((0, paths_js_2.resolveLegacyAuthStorePath)(agentDir));
    var legacy = coerceLegacyStore(legacyRaw);
    var store = {
        version: constants_js_1.AUTH_STORE_VERSION,
        profiles: {},
    ...
    if (legacy) {
        for (var _i = 0, _d = Object.entries(legacy); _i < _d.length; _i++) {
            ...
            var profileId = "".concat(provider, ":default");
            if (cred.type === "api_key") {
                store.profiles[profileId] = __assign({ type: "api_key", provider: String((_a = cred.provider) !== null && _a !== void 0 ? _a : provider), key: cred.key }, (cred.email ? { email: cred.email } : {}));
            }
            else if (cred.type === "token") {
                store.profiles[profileId] = __assign(__assign({ type: "token", provider: String((_b = cred.provider) !== null && _b !== void 0 ? _b : provider), token: cred.token }, (typeof cred.expires === "number" ? { expires: cred.expires } : {})), (cred.email ? { email: cred.email } : {}));
            }
            else {
