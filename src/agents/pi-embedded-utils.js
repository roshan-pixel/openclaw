...
function stripDowngradedToolCallText(text) {
    ...
    var consumeJsonish = function (input, start, options) {
        var _a = (options !== null && options !== void 0 ? options : {}).allowLeadingNewlines, allowLeadingNewlines = _a === void 0 ? false : _a;
        var index = start;
...
function stripThinkingTagsFromText(text) {
    return (0, reasoning_tags_js_1.stripReasoningTagsFromText)(text, { mode: "strict", trim: "both" });
}
function extractAssistantText(msg) {
    ...
    var blocks = Array.isArray(msg.content)
        ? msg.content
            .filter(isTextBlock)
            .map(function (c) {
            return stripThinkingTagsFromText(stripDowngradedToolCallText(stripMinimaxToolCallXml(c.text))).trim();
        })
            .filter(Boolean)
        : [];
...
function formatReasoningMessage(text) {
    ...
    var italicLines = trimmed
        .split("\n")
        .map(function (line) { return (line ? "_".concat(line, "_") : line); })
        .join("\n");
    return "Reasoning:\n".concat(italicLines);
}
...
function promoteThinkingTagsToBlocks(message) {
    if (!Array.isArray(message.content))
        return;
    var hasThinkingBlock = message.content.some(function (block) { return block.type === "thinking"; });
    if (hasThinkingBlock)
        return;
    var next = [];
    var changed = false;
...
function extractThinkingFromTaggedStream(text) {
    ...
    if (lastClose && ((_a = lastClose.index) !== null && _a !== void 0 ? _a : -1) > ((_b = lastOpen.index) !== null && _b !== void 0 ? _b : -1)) {
        return closed;
    }
