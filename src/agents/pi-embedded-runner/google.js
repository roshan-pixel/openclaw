...
function sanitizeAntigravityThinkingBlocks(messages) {
    ...
    for (var _i = 0, messages_1 = messages; _i < messages_1.length; _i++) {
        ...
        for (var _d = 0, _e = assistant.content; _d < _e.length; _d++) {
            ...
            var rec = block;
            var candidate = (_c = (_b = (_a = rec.thinkingSignature) !== null && _a !== void 0 ? _a : rec.signature) !== null && _b !== void 0 ? _b : rec.thought_signature) !== null && _c !== void 0 ? _c : rec.thoughtSignature;
...
function findUnsupportedSchemaKeywords(schema, path) {
    ...
    var record = schema;
    var violations = [];
    var properties = record.properties && typeof record.properties === "object" && !Array.isArray(record.properties)
        ? record.properties
        : undefined;
    if (properties) {
        for (var _i = 0, _a = Object.entries(properties); _i < _a.length; _i++) {
            var _b = _a[_i], key = _b[0], value = _b[1];
            violations.push.apply(violations, findUnsupportedSchemaKeywords(value, "".concat(path, ".properties.").concat(key)));
        }
    }
    for (var _c = 0, _d = Object.entries(record); _c < _d.length; _c++) {
        ...
        if (value && typeof value === "object") {
            violations.push.apply(violations, findUnsupportedSchemaKeywords(value, "".concat(path, ".").concat(key)));
        }
    }
    return violations;
}
function sanitizeToolsForGoogle(params) {
    ...
    return params.tools.map(function (tool) {
        if (!tool.parameters || typeof tool.parameters !== "object")
            return tool;
        return __assign(__assign({}, tool), { parameters: (0, pi_tools_schema_js_1.cleanToolSchemaForGemini)(tool.parameters) });
    });
}
function logToolSchemasForGoogle(params) {
    ...
    var toolNames = params.tools.map(function (tool, index) { return "".concat(index, ":").concat(tool.name); });
    ...
    for (var _i = 0, _a = tools.entries(); _i < _a.length; _i++) {
        var _b = _a[_i], index = _b[0], tool = _b[1];
        var violations = findUnsupportedSchemaKeywords(tool.parameters, "".concat(tool.name, ".parameters"));
...
function onUnhandledCompactionFailure(cb) {
    ...
    return function () { return compactionFailureEmitter.off("failure", cb); };
}
...
function readLastModelSnapshot(sessionManager) {
    try {
        ...
        for (var i = entries.length - 1; i >= 0; i--) {
            var entry = entries[i];
            if ((entry === null || entry === void 0 ? void 0 : entry.type) !== "custom" || (entry === null || entry === void 0 ? void 0 : entry.customType) !== MODEL_SNAPSHOT_CUSTOM_TYPE)
                continue;
...
function isSameModelSnapshot(a, b) {
    var normalize = function (value) { return value !== null && value !== void 0 ? value : ""; };
...
function hasGoogleTurnOrderingMarker(sessionManager) {
    try {
        return sessionManager
            .getEntries()
            .some(function (entry) {
            return (entry === null || entry === void 0 ? void 0 : entry.type) === "custom" &&
                (entry === null || entry === void 0 ? void 0 : entry.customType) === GOOGLE_TURN_ORDERING_CUSTOM_TYPE;
        });
    }
    catch (_a) {
        return false;
...
function applyGoogleTurnOrderingFix(params) {
    ...
    if (didPrepend && !hasGoogleTurnOrderingMarker(params.sessionManager)) {
        var warn = (_a = params.warn) !== null && _a !== void 0 ? _a : (function (message) { return logger_js_1.log.warn(message); });
        warn("google turn ordering fixup: prepended user bootstrap (sessionId=".concat(params.sessionId, ")"));
...
function sanitizeSessionHistory(params) {
    return __awaiter(this, void 0, void 0, function () {
        var policy, sanitizedImages, sanitizedThinking, repairedTools, isOpenAIResponsesApi, hasSnapshot, priorSnapshot, modelChanged, sanitizedOpenAI;
        var _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    policy = (_a = params.policy) !== null && _a !== void 0 ? _a : (0, transcript_policy_js_1.resolveTranscriptPolicy)({
                        modelApi: params.modelApi,
                        provider: params.provider,
                        modelId: params.modelId,
                    });
                    return [4 /*yield*/, (0, pi_embedded_helpers_js_1.sanitizeSessionMessagesImages)(params.messages, "session:history", {
                ...
                case 1:
                    ...
                    repairedTools = policy.repairToolUseResultPairing
                        ...
                        : sanitizedThinking;
                    isOpenAIResponsesApi = params.modelApi === "openai-responses" || params.modelApi === "openai-codex-responses";
                    ...
                    modelChanged = priorSnapshot
                        ...
                        : false;
                    sanitizedOpenAI = isOpenAIResponsesApi && modelChanged
                    ...
                    return [2 /*return*/, applyGoogleTurnOrderingFix({
