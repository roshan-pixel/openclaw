...
function mergePropertySchemas(existing, incoming) {
    ...
    if (existingEnum || incomingEnum) {
        var values = Array.from(new Set(__spreadArray(__spreadArray([], (existingEnum !== null && existingEnum !== void 0 ? existingEnum : []), true), (incomingEnum !== null && incomingEnum !== void 0 ? incomingEnum : []), true)));
        var merged = {};
        ...
        var types = new Set(values.map(function (value) { return typeof value; }));
        if (types.size === 1)
...
function normalizeToolParameters(tool) {
    ...
    if ("type" in schema && "properties" in schema && !Array.isArray(schema.anyOf)) {
        return __assign(__assign({}, tool), { parameters: (0, clean_for_gemini_js_1.cleanSchemaForGemini)(schema) });
    }
    ...
    if (!("type" in schema) &&
        ...
        return __assign(__assign({}, tool), { parameters: (0, clean_for_gemini_js_1.cleanSchemaForGemini)(__assign(__assign({}, schema), { type: "object" })) });
    }
    ...
    for (var _i = 0, variants_1 = variants; _i < variants_1.length; _i++) {
        ...
        var required = Array.isArray(entry.required)
            ? entry.required
            : [];
        for (var _f = 0, required_1 = required; _f < required_1.length; _f++) {
            ...
            if (typeof key !== "string")
                continue;
            requiredCounts.set(key, ((_a = requiredCounts.get(key)) !== null && _a !== void 0 ? _a : 0) + 1);
        }
    }
    var baseRequired = Array.isArray(schema.required)
        ? schema.required.filter(function (key) { return typeof key === "string"; })
        : undefined;
    var mergedRequired = baseRequired && baseRequired.length > 0
        ? baseRequired
        : objectVariants > 0
    ...
    return __assign(__assign({}, tool), { 
        ...
        parameters: (0, clean_for_gemini_js_1.cleanSchemaForGemini)(__assign(__assign(__assign(__assign(__assign({ type: "object" }, (typeof nextSchema.title === "string" ? { title: nextSchema.title } : {})), (typeof nextSchema.description === "string"
            ? { description: nextSchema.description }
            : {})), { properties: Object.keys(mergedProperties).length > 0 ? mergedProperties : ((_b = schema.properties) !== null && _b !== void 0 ? _b : {}) }), (mergedRequired && mergedRequired.length > 0 ? { required: mergedRequired } : {})), { additionalProperties: "additionalProperties" in schema ? schema.additionalProperties : true })) });
}
