...
var _EmbeddedBlockChunker_instances, _EmbeddedBlockChunker_buffer, _EmbeddedBlockChunker_chunking, _EmbeddedBlockChunker_pickSoftBreakIndex, _EmbeddedBlockChunker_pickBreakIndex;
...
var EmbeddedBlockChunker = /** @class */ (function () {
    ...
    EmbeddedBlockChunker.prototype.append = function (text) {
        if (!text)
            return;
        __classPrivateFieldSet(this, _EmbeddedBlockChunker_buffer, __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f") + text, "f");
    };
    ...
    EmbeddedBlockChunker.prototype.drain = function (params) {
        ...
        var force = params.force, emit = params.emit;
        var minChars = Math.max(1, Math.floor(__classPrivateFieldGet(this, _EmbeddedBlockChunker_chunking, "f").minChars));
        var maxChars = Math.max(minChars, Math.floor(__classPrivateFieldGet(this, _EmbeddedBlockChunker_chunking, "f").maxChars));
        if (__classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length < minChars && !force)
            return;
        if (force && __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length <= maxChars) {
        ...
        while (__classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length >= minChars || (force && __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length > 0)) {
            var breakResult = force && __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length <= maxChars
                ? __classPrivateFieldGet(this, _EmbeddedBlockChunker_instances, "m", _EmbeddedBlockChunker_pickSoftBreakIndex).call(this, __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f"), 1)
                : __classPrivateFieldGet(this, _EmbeddedBlockChunker_instances, "m", _EmbeddedBlockChunker_pickBreakIndex).call(this, __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f"), force ? 1 : undefined);
            ...
            var rawChunk = __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").slice(0, breakIdx);
            if (rawChunk.trim().length === 0) {
                __classPrivateFieldSet(this, _EmbeddedBlockChunker_buffer, stripLeadingNewlines(__classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").slice(breakIdx)).trimStart(), "f");
                continue;
            }
            var nextBuffer = __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").slice(breakIdx);
            ...
            if (fenceSplit) {
                ...
                var nextStart = breakIdx < __classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length && /\s/.test(__classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f")[breakIdx])
                    ? breakIdx + 1
                    : breakIdx;
                __classPrivateFieldSet(this, _EmbeddedBlockChunker_buffer, stripLeadingNewlines(__classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").slice(nextStart)), "f");
            }
            if (__classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length < minChars && !force)
                return;
            if (__classPrivateFieldGet(this, _EmbeddedBlockChunker_buffer, "f").length < maxChars && !force)
                return;
        }
    };
...
_EmbeddedBlockChunker_buffer = new WeakMap(), _EmbeddedBlockChunker_chunking = new WeakMap(), _EmbeddedBlockChunker_instances = new WeakSet(), _EmbeddedBlockChunker_pickSoftBreakIndex = function _EmbeddedBlockChunker_pickSoftBreakIndex(buffer, minCharsOverride) {
    var _a, _b;
    var minChars = Math.max(1, Math.floor(minCharsOverride !== null && minCharsOverride !== void 0 ? minCharsOverride : __classPrivateFieldGet(this, _EmbeddedBlockChunker_chunking, "f").minChars));
    if (buffer.length < minChars)
        return { index: -1 };
    var fenceSpans = (0, fences_js_1.parseFenceSpans)(buffer);
    var preference = (_a = __classPrivateFieldGet(this, _EmbeddedBlockChunker_chunking, "f").breakPreference) !== null && _a !== void 0 ? _a : "paragraph";
    ...
    return { index: -1 };
}, _EmbeddedBlockChunker_pickBreakIndex = function _EmbeddedBlockChunker_pickBreakIndex(buffer, minCharsOverride) {
    var _a, _b;
    var minChars = Math.max(1, Math.floor(minCharsOverride !== null && minCharsOverride !== void 0 ? minCharsOverride : __classPrivateFieldGet(this, _EmbeddedBlockChunker_chunking, "f").minChars));
    var maxChars = Math.max(minChars, Math.floor(__classPrivateFieldGet(this, _EmbeddedBlockChunker_chunking, "f").maxChars));
    if (buffer.length < minChars)
