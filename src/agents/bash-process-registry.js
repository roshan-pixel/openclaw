...
var jobTtlMs = clampTtl(Number.parseInt((_a = process.env.PI_BASH_JOB_TTL_MS) !== null && _a !== void 0 ? _a : "", 10));
...
function appendOutput(session, stream, chunk) {
    ...
    (_c = session.pendingStdoutChars) !== null && _c !== void 0 ? _c : (session.pendingStdoutChars = sumPendingChars(session.pendingStdout));
    (_d = session.pendingStderrChars) !== null && _d !== void 0 ? _d : (session.pendingStderrChars = sumPendingChars(session.pendingStderr));
    ...
    var pendingCap = Math.min((_e = session.pendingMaxOutputChars) !== null && _e !== void 0 ? _e : DEFAULT_PENDING_OUTPUT_CHARS, session.maxOutputChars);
    buffer.push(chunk);
...
function listRunningSessions() {
    return Array.from(runningSessions.values()).filter(function (s) { return s.backgrounded; });
}
