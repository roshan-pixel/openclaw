...
function withNormalizedTimestamp(value, rawTimestamp) {
    ...
    if (!normalized)
        return value;
    return __assign(__assign({}, value), { timestampMs: typeof value.timestampMs === "number" && Number.isFinite(value.timestampMs)
            ? value.timestampMs
            : normalized.timestampMs, timestampUtc: typeof value.timestampUtc === "string" && value.timestampUtc.trim()
            ? value.timestampUtc
...
function detectSystemTimeFormat() {
    if (process.platform === "darwin") {
        try {
            var result = (0, node_child_process_1.execSync)("defaults read -g AppleICUForce24HourTime 2>/dev/null", {
                encoding: "utf8",
                timeout: 500,
            }).trim();
            if (result === "1")
    ...
    if (process.platform === "win32") {
        try {
            var result = (0, node_child_process_1.execSync)('powershell -Command "(Get-Culture).DateTimeFormat.ShortTimePattern"', { encoding: "utf8", timeout: 1000 }).trim();
...
function formatUserTime(date, timeZone, format) {
    ...
    try {
        ...
        var timePart = use24Hour
            ...
            : "".concat(map.hour, ":").concat(map.minute, " ").concat((_a = map.dayPeriod) !== null && _a !== void 0 ? _a : "").trim();
        return "".concat(map.weekday, ", ").concat(map.month, " ").concat(dayNum).concat(suffix, ", ").concat(map.year, " \u2014 ").concat(timePart);
    }
    catch (_b) {
        return undefined;
    }
