            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    if (!sniffed.startsWith("image/")) {
                        throw new Error("read: file looks like ".concat(sniffed, " but was treated as ").concat(image.mimeType, " (").concat(filePath, ")"));
                    }
...
function patchToolSchemaForClaudeCompatibility(tool) {
    ...
    var required = Array.isArray(schema.required)
        ? schema.required.filter(function (key) { return typeof key === "string"; })
        : [];
    var changed = false;
    var aliasPairs = [
    ...
    for (var _i = 0, aliasPairs_1 = aliasPairs; _i < aliasPairs_1.length; _i++) {
        var _a = aliasPairs_1[_i], original = _a.original, alias = _a.alias;
    ...
    if (!changed)
        return tool;
    return __assign(__assign({}, tool), { parameters: __assign(__assign(__assign({}, schema), { properties: properties }), (required.length > 0 ? { required: required } : {})) });
}
...
function wrapToolParamNormalization(tool, requiredParamGroups) {
    ...
    return __assign(__assign({}, patched), { execute: function (toolCallId, params, signal, onUpdate) { return __awaiter(_this, void 0, void 0, function () {
            var normalized, record;
            return __generator(this, function (_a) {
                normalized = normalizeToolParams(params);
                record = normalized !== null && normalized !== void 0 ? normalized : (params && typeof params === "object" ? params : undefined);
                if (requiredParamGroups === null || requiredParamGroups === void 0 ? void 0 : requiredParamGroups.length) {
                    assertRequiredParams(record, requiredParamGroups, tool.name);
                }
                return [2 /*return*/, tool.execute(toolCallId, normalized !== null && normalized !== void 0 ? normalized : params, signal, onUpdate)];
            });
        }); } });
}
function wrapSandboxPathGuard(tool, root) {
    var _this = this;
    return __assign(__assign({}, tool), { execute: function (toolCallId, args, signal, onUpdate) { return __awaiter(_this, void 0, void 0, function () {
            var normalized, record, filePath;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        normalized = normalizeToolParams(args);
                        record = normalized !== null && normalized !== void 0 ? normalized : (args && typeof args === "object" ? args : undefined);
                        filePath = record === null || record === void 0 ? void 0 : record.path;
                        if (!(typeof filePath === "string" && filePath.trim())) return [3 /*break*/, 2];
                        return [4 /*yield*/, (0, sandbox_paths_js_1.assertSandboxPath)({ filePath: filePath, cwd: root, root: root })];
                    case 1:
                        _a.sent();
                        _a.label = 2;
                    case 2: return [2 /*return*/, tool.execute(toolCallId, normalized !== null && normalized !== void 0 ? normalized : args, signal, onUpdate)];
                }
            });
        }); } });
}
...
function createSandboxedWriteTool(root) {
    ...
    return wrapSandboxPathGuard(wrapToolParamNormalization(base, exports.CLAUDE_PARAM_GROUPS.write), root);
}
function createSandboxedEditTool(root) {
    var base = (0, pi_coding_agent_1.createEditTool)(root);
    return wrapSandboxPathGuard(wrapToolParamNormalization(base, exports.CLAUDE_PARAM_GROUPS.edit), root);
}
function createOpenClawReadTool(base) {
    var _this = this;
    var patched = patchToolSchemaForClaudeCompatibility(base);
    return __assign(__assign({}, patched), { execute: function (toolCallId, params, signal) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, base.execute(toolCallId, normalized !== null && normalized !== void 0 ? normalized : params, signal)];
                    case 1:
                        result = (_a.sent());
                        filePath = typeof (record === null || record === void 0 ? void 0 : record.path) === "string" ? String(record.path) : "<unknown>";
                        return [4 /*yield*/, normalizeReadImageResult(result, filePath)];
                    case 2:
                        normalizedResult = _a.sent();
                        return [2 /*return*/, (0, tool_images_js_1.sanitizeToolResultImages)(normalizedResult, "read:".concat(filePath))];
                }
            });
        }); } });
}
