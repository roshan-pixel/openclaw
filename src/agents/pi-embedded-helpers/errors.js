...
function isContextOverflowError(errorMessage) {
    ...
    var hasContextWindow = lower.includes("context window") ||
...
var CONTEXT_OVERFLOW_HINT_RE = /context.*overflow|context window.*(too (?:large|long)|exceed|over|limit|max(?:imum)?|requested|sent|tokens)|(?:prompt|request|input).*(too (?:large|long)|exceed|over|limit|max(?:imum)?)/i;
...
var ERROR_PAYLOAD_PREFIX_RE = /^(?:error|api\s*error|apierror|openai\s*error|anthropic\s*error|gateway\s*error)[:\s-]+/i;
var FINAL_TAG_RE = /<\s*\/?\s*final\s*>/gi;
var ERROR_PREFIX_RE = /^(?:error|api\s*error|openai\s*error|anthropic\s*error|gateway\s*error|request failed|failed|exception)[:\s-]+/i;
...
function collapseConsecutiveDuplicateBlocks(text) {
    ...
    if (blocks.length < 2)
        return text;
    var normalizeBlock = function (value) { return value.trim().replace(/\s+/g, " "); };
    var result = [];
...
function isLikelyHttpErrorText(raw) {
    ...
    return HTTP_ERROR_HINTS.some(function (hint) { return message.includes(hint); });
}
...
function parseApiErrorInfo(raw) {
    ...
    if (!payload)
        return null;
    var requestId = typeof payload.request_id === "string"
        ? payload.request_id
...
function formatAssistantErrorText(msg, opts) {
    ...
    if (!raw)
        return "LLM request failed with an unknown error.";
    var unknownTool = (_b = raw.match(/unknown tool[:\s]+["']?([a-z0-9_-]+)["']?/i)) !== null && _b !== void 0 ? _b : raw.match(/tool\s+["']?([a-z0-9_-]+)["']?\s+(?:not found|is not available)/i);
...
var IMAGE_DIMENSION_ERROR_RE = /image dimensions exceed max allowed size for many-image requests:\s*(\d+)\s*pixels/i;
...
function parseImageDimensionError(raw) {
    ...
    return {
        maxDimensionPx: (limitMatch === null || limitMatch === void 0 ? void 0 : limitMatch[1]) ? Number.parseInt(limitMatch[1], 10) : undefined,
        messageIndex: (pathMatch === null || pathMatch === void 0 ? void 0 : pathMatch[1]) ? Number.parseInt(pathMatch[1], 10) : undefined,
        contentIndex: (pathMatch === null || pathMatch === void 0 ? void 0 : pathMatch[2]) ? Number.parseInt(pathMatch[2], 10) : undefined,
        raw: raw,
    };
}
...
function parseImageSizeError(raw) {
    ...
    return {
        maxMb: (match === null || match === void 0 ? void 0 : match[1]) ? Number.parseFloat(match[1]) : undefined,
        raw: raw,
    };
}
