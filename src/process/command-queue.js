    ...
    var pump = function () {
        ...
        var _loop_1 = function () {
            ...
            if (waitedMs >= entry.warnAfterMs) {
                (_a = entry.onWait) === null || _a === void 0 ? void 0 : _a.call(entry, waitedMs, state.queue.length);
                diagnostic_js_1.diagnosticLogger.warn("lane wait exceeded: lane=".concat(lane, " waitedMs=").concat(waitedMs, " queueAhead=").concat(state.queue.length));
            }
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        ...
                        case 2:
                            result = _a.sent();
                            state.active -= 1;
                            diagnostic_js_1.diagnosticLogger.debug("lane task done: lane=".concat(lane, " durationMs=").concat(Date.now() - startTime, " active=").concat(state.active, " queued=").concat(state.queue.length));
                            pump();
                        ...
                        case 3:
                            ...
                            if (!isProbeLane) {
                                diagnostic_js_1.diagnosticLogger.error("lane task error: lane=".concat(lane, " durationMs=").concat(Date.now() - startTime, " error=\"").concat(String(err_1), "\""));
                            }
                            pump();
                            entry.reject(err_1);
                            return [3 /*break*/, 4];
...
function enqueueCommandInLane(lane, task, opts) {
    var _a;
    var cleaned = lane.trim() || "main" /* CommandLane.Main */;
    var warnAfterMs = (_a = opts === null || opts === void 0 ? void 0 : opts.warnAfterMs) !== null && _a !== void 0 ? _a : 2000;
    var state = getLaneState(cleaned);
...
function getQueueSize(lane) {
    if (lane === void 0) { lane = "main" /* CommandLane.Main */; }
