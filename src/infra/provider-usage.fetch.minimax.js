...
function hasAny(record, keys) {
    return keys.some(function (key) { return key in record; });
}
...
function collectUsageCandidates(root) {
    ...
    while (queue.length && scanned < MAX_SCAN_NODES) {
        ...
        if (!next)
            break;
        scanned += 1;
        var value = next.value, depth = next.depth;
        if (isRecord(value)) {
            if (seen.has(value))
                continue;
            seen.add(value);
    ...
    return candidates.map(function (candidate) { return candidate.record; });
}
...
function deriveUsedPercent(payload) {
    ...
    var fromCounts = total && total > 0 && used !== undefined && Number.isFinite(used)
    ...
    if (percentRaw !== undefined) {
        var normalized = (0, provider_usage_shared_js_1.clampPercent)(percentRaw <= 1 ? percentRaw * 100 : percentRaw);
        if (fromCounts !== null) {
...
function fetchMinimaxUsage(apiKey, timeoutMs, fetchFn) {
    return __awaiter(this, void 0, void 0, function () {
        var res, data, baseResp, payload, candidates, usageRecord, usedPercent, _i, candidates_1, candidate, candidatePercent, resetAt, windows;
        var _a, _b, _c, _d, _e;
        return __generator(this, function (_f) {
            switch (_f.label) {
                case 0: return [4 /*yield*/, (0, provider_usage_fetch_shared_js_1.fetchJson)("https://api.minimaxi.com/v1/api/openplatform/coding_plan/remains", {
                        method: "GET",
                        headers: {
                ...
                case 2:
                    ...
                    if (baseResp && typeof baseResp.status_code === "number" && baseResp.status_code !== 0) {
                        return [2 /*return*/, {
                                ...
                                displayName: provider_usage_shared_js_1.PROVIDER_LABELS.minimax,
                                windows: [],
                                error: ((_a = baseResp.status_msg) === null || _a === void 0 ? void 0 : _a.trim()) || "API error",
                            }];
                    }
                    ...
                    resetAt = (_d = (_c = (_b = parseEpoch(pickString(usageRecord, RESET_KEYS))) !== null && _b !== void 0 ? _b : parseEpoch(pickNumber(usageRecord, RESET_KEYS))) !== null && _c !== void 0 ? _c : parseEpoch(pickString(payload, RESET_KEYS))) !== null && _d !== void 0 ? _d : parseEpoch(pickNumber(payload, RESET_KEYS));
                    windows = [
                        {
                    ...
                    return [2 /*return*/, {
                            ...
                            displayName: provider_usage_shared_js_1.PROVIDER_LABELS.minimax,
                            windows: windows,
                            plan: (_e = pickString(usageRecord, PLAN_KEYS)) !== null && _e !== void 0 ? _e : pickString(payload, PLAN_KEYS),
                        }];
            }
        });
    });
