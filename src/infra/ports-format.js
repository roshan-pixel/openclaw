...
function classifyPortListener(listener, port) {
    var _a, _b;
    var raw = "".concat((_a = listener.commandLine) !== null && _a !== void 0 ? _a : "", " ").concat((_b = listener.command) !== null && _b !== void 0 ? _b : "").trim().toLowerCase();
    if (raw.includes("openclaw")) {
        return "gateway";
    }
    if (raw.includes("ssh")) {
        var portToken = String(port);
        var tunnelPattern = new RegExp("-(l|r)\\s*".concat(portToken, "\\b|-(l|r)").concat(portToken, "\\b|:").concat(portToken, "\\b"));
...
function buildPortHints(listeners, port) {
    if (listeners.length === 0)
        return [];
    var kinds = new Set(listeners.map(function (listener) { return classifyPortListener(listener, port); }));
    var hints = [];
    if (kinds.has("gateway")) {
        hints.push("Gateway already running locally. Stop it (".concat((0, command_format_js_1.formatCliCommand)("openclaw gateway stop"), ") or use a different port."));
    }
    if (kinds.has("ssh")) {
