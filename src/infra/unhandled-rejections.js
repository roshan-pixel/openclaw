...
function isTransientNetworkError(err) {
    ...
    if (err instanceof AggregateError && ((_a = err.errors) === null || _a === void 0 ? void 0 : _a.length)) {
        return err.errors.some(function (e) { return isTransientNetworkError(e); });
    }
    return false;
}
...
function isUnhandledRejectionHandled(reason) {
    ...
    for (var _i = 0, handlers_1 = handlers; _i < handlers_1.length; _i++) {
        ...
        try {
            ...
            console.error("[openclaw] Unhandled rejection handler failed:", err instanceof Error ? ((_a = err.stack) !== null && _a !== void 0 ? _a : err.message) : err);
        }
    }
    return false;
}
function installUnhandledRejectionHandler() {
    node_process_1.default.on("unhandledRejection", function (reason, _promise) {
        ...
        if (isAbortError(reason)) {
            console.warn("[openclaw] Suppressed AbortError:", (0, errors_js_1.formatUncaughtError)(reason));
            return;
        }
        if (isFatalError(reason)) {
            console.error("[openclaw] FATAL unhandled rejection:", (0, errors_js_1.formatUncaughtError)(reason));
            node_process_1.default.exit(1);
            return;
        }
        if (isConfigError(reason)) {
            console.error("[openclaw] CONFIGURATION ERROR - requires fix:", (0, errors_js_1.formatUncaughtError)(reason));
            node_process_1.default.exit(1);
            return;
        }
        if (isTransientNetworkError(reason)) {
            console.warn("[openclaw] Non-fatal unhandled rejection (continuing):", (0, errors_js_1.formatUncaughtError)(reason));
            return;
        }
        console.error("[openclaw] Unhandled promise rejection:", (0, errors_js_1.formatUncaughtError)(reason));
        node_process_1.default.exit(1);
    });
}
