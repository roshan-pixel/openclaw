...
function pickPrimaryWindow(windows) {
    if (windows.length === 0)
        return undefined;
    return windows.reduce(function (best, next) { return (next.usedPercent > best.usedPercent ? next : best); });
}
...
function formatUsageWindowSummary(snapshot, opts) {
    ...
    if (snapshot.windows.length === 0)
        return null;
    var now = (_a = opts === null || opts === void 0 ? void 0 : opts.now) !== null && _a !== void 0 ? _a : Date.now();
    var maxWindows = typeof (opts === null || opts === void 0 ? void 0 : opts.maxWindows) === "number" && opts.maxWindows > 0
        ? Math.min(opts.maxWindows, snapshot.windows.length)
        : snapshot.windows.length;
    var includeResets = (_b = opts === null || opts === void 0 ? void 0 : opts.includeResets) !== null && _b !== void 0 ? _b : false;
...
function formatUsageSummaryLine(summary, opts) {
    ...
    var providers = summary.providers
        .filter(function (entry) { return entry.windows.length > 0 && !entry.error; })
        .slice(0, (_a = opts === null || opts === void 0 ? void 0 : opts.maxProviders) !== null && _a !== void 0 ? _a : summary.providers.length);
    ...
    var parts = providers
        ...
        if (!window)
            return null;
        return "".concat(entry.displayName, " ").concat(formatWindowShort(window, opts === null || opts === void 0 ? void 0 : opts.now));
    })
        .filter(Boolean);
    if (parts.length === 0)
        return null;
...
function formatUsageReportLines(summary, opts) {
    ...
    for (var _i = 0, _a = summary.providers; _i < _a.length; _i++) {
        ...
        for (var _b = 0, _c = entry.windows; _b < _c.length; _b++) {
            ...
            var reset = formatResetRemaining(window_1.resetAt, opts === null || opts === void 0 ? void 0 : opts.now);
            var resetSuffix = reset ? " \u00B7 resets ".concat(reset) : "";
            lines.push("    ".concat(window_1.label, ": ").concat(remaining.toFixed(0), "% left").concat(resetSuffix));
        }
    }
    return lines;
}
