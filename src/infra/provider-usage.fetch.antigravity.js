    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                ...
                case 1:
                    data = (_d.sent());
                    message = (_c = (_b = data === null || data === void 0 ? void 0 : data.error) === null || _b === void 0 ? void 0 : _b.message) === null || _c === void 0 ? void 0 : _c.trim();
                    if (message)
                ...
                case 2:
                    _a = _d.sent();
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/, "HTTP ".concat(res.status)];
            }
        });
    });
}
function extractCredits(data) {
    ...
    var monthly = parseNumber((_a = data.planInfo) === null || _a === void 0 ? void 0 : _a.monthlyPromptCredits);
...
function extractPlanInfo(data) {
    var _a, _b, _c;
    var tierName = (_b = (_a = data.currentTier) === null || _a === void 0 ? void 0 : _a.name) === null || _b === void 0 ? void 0 : _b.trim();
    if (tierName)
        return tierName;
...
function extractModelQuotas(data) {
    ...
    for (var _i = 0, _a = Object.entries(data.models); _i < _a.length; _i++) {
        var _b = _a[_i], modelId = _b[0], modelInfo = _b[1];
...
function buildUsageWindows(opts) {
    ...
    if (opts.credits) {
        var _a = opts.credits, available = _a.available, monthly = _a.monthly;
    ...
    if (opts.modelQuotas && opts.modelQuotas.size > 0) {
        ...
        for (var _i = 0, _b = opts.modelQuotas; _i < _b.length; _i++) {
            var _c = _b[_i], modelId = _c[0], quota = _c[1];
            var lowerModelId = modelId.toLowerCase();
            // Skip internal models
            if (lowerModelId.includes("chat_") || lowerModelId.includes("tab_")) {
                continue;
            }
            var usedPercent = (0, provider_usage_shared_js_1.clampPercent)((1 - quota.remainingFraction) * 100);
        ...
        modelWindows.sort(function (a, b) { return b.usedPercent - a.usedPercent; });
        var topModels = modelWindows.slice(0, 10);
        (0, logger_js_1.logDebug)("[antigravity] Built ".concat(topModels.length, " model windows from ").concat(opts.modelQuotas.size, " total models"));
        for (var _d = 0, topModels_1 = topModels; _d < topModels_1.length; _d++) {
            var w = topModels_1[_d];
            (0, logger_js_1.logDebug)("[antigravity]   ".concat(w.label, ": ").concat(w.usedPercent.toFixed(1), "% used").concat(w.resetAt ? " (resets at ".concat(new Date(w.resetAt).toISOString(), ")") : ""));
        }
...
function fetchAntigravityUsage(token, timeoutMs, fetchFn) {
    return __awaiter(this, void 0, void 0, function () {
        var headers, credits, modelQuotas, planInfo, lastError, projectId, res, data, _a, body, res, data, _i, modelQuotas_1, _b, modelId, quota, err, _c, windows, snapshot;
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    headers = {
                ...
                case 1:
                    _d.trys.push([1, 7, , 8]);
                    return [4 /*yield*/, (0, provider_usage_fetch_shared_js_1.fetchJson)("".concat(BASE_URL).concat(LOAD_CODE_ASSIST_PATH), { method: "POST", headers: headers, body: JSON.stringify({ metadata: METADATA }) }, timeoutMs, fetchFn)];
                case 2:
                    res = _d.sent();
                ...
                case 3:
                    ...
                    (0, logger_js_1.logDebug)("[antigravity] Credits: ".concat(credits ? "".concat(credits.available, "/").concat(credits.monthly) : "none").concat(planInfo ? " (plan: ".concat(planInfo, ")") : ""));
                    return [3 /*break*/, 6];
                case 4: return [4 /*yield*/, parseErrorMessage(res)];
                case 5:
                    lastError = _d.sent();
                ...
                case 8:
                    ...
                    if (!projectId) {
                        (0, logger_js_1.logDebug)("[antigravity] Missing project id; requesting available models without project");
                    }
                    _d.label = 9;
                case 9:
                    _d.trys.push([9, 15, , 16]);
                    body = JSON.stringify(projectId ? { project: projectId } : {});
                    return [4 /*yield*/, (0, provider_usage_fetch_shared_js_1.fetchJson)("".concat(BASE_URL).concat(FETCH_AVAILABLE_MODELS_PATH), { method: "POST", headers: headers, body: body }, timeoutMs, fetchFn)];
                case 10:
                    res = _d.sent();
                ...
                case 11:
                    ...
                    (0, logger_js_1.logDebug)("[antigravity] Extracted ".concat(modelQuotas.size, " model quotas from API"));
                    for (_i = 0, modelQuotas_1 = modelQuotas; _i < modelQuotas_1.length; _i++) {
                        _b = modelQuotas_1[_i], modelId = _b[0], quota = _b[1];
                        (0, logger_js_1.logDebug)("[antigravity]   ".concat(modelId, ": ").concat((quota.remainingFraction * 100).toFixed(1), "% remaining").concat(quota.resetTime ? " (resets ".concat(new Date(quota.resetTime).toISOString(), ")") : ""));
                    }
                    return [3 /*break*/, 14];
                case 12: return [4 /*yield*/, parseErrorMessage(res)];
                case 13:
                    err = _d.sent();
                    if (res.status === 401) {
                ...
                case 16:
                    ...
                    (0, logger_js_1.logDebug)("[antigravity] Returning snapshot with ".concat(windows.length, " windows").concat(planInfo ? " (plan: ".concat(planInfo, ")") : ""));
                    (0, logger_js_1.logDebug)("[antigravity] Snapshot: ".concat(JSON.stringify(snapshot, null, 2)));
