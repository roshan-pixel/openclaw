...
function describeNode(nodeId) {
    ...
    var name = (_a = record === null || record === void 0 ? void 0 : record.displayName) === null || _a === void 0 ? void 0 : _a.trim();
    var base = name && name !== nodeId ? "".concat(name, " (").concat(nodeId, ")") : nodeId;
    var ip = (_b = record === null || record === void 0 ? void 0 : record.remoteIp) === null || _b === void 0 ? void 0 : _b.trim();
...
function logRemoteBinProbeFailure(nodeId, err) {
    ...
    if ((message === null || message === void 0 ? void 0 : message.includes("node not connected")) || (message === null || message === void 0 ? void 0 : message.includes("node disconnected"))) {
        log.info("remote bin probe skipped: node unavailable (".concat(label, ")"));
        return;
    }
    if ((message === null || message === void 0 ? void 0 : message.includes("invoke timed out")) || (message === null || message === void 0 ? void 0 : message.includes("timeout"))) {
        log.warn("remote bin probe timed out (".concat(label, "); check node connectivity for ").concat(label));
        return;
    }
    log.warn("remote bin probe error (".concat(label, "): ").concat(message !== null && message !== void 0 ? message : "unknown"));
}
...
function upsertNode(record) {
    ...
    var bins = new Set((_b = (_a = record.bins) !== null && _a !== void 0 ? _a : existing === null || existing === void 0 ? void 0 : existing.bins) !== null && _b !== void 0 ? _b : []);
    remoteNodes.set(record.nodeId, {
        nodeId: record.nodeId,
        displayName: (_c = record.displayName) !== null && _c !== void 0 ? _c : existing === null || existing === void 0 ? void 0 : existing.displayName,
        platform: (_d = record.platform) !== null && _d !== void 0 ? _d : existing === null || existing === void 0 ? void 0 : existing.platform,
        deviceFamily: (_e = record.deviceFamily) !== null && _e !== void 0 ? _e : existing === null || existing === void 0 ? void 0 : existing.deviceFamily,
        commands: (_f = record.commands) !== null && _f !== void 0 ? _f : existing === null || existing === void 0 ? void 0 : existing.commands,
        remoteIp: (_g = record.remoteIp) !== null && _g !== void 0 ? _g : existing === null || existing === void 0 ? void 0 : existing.remoteIp,
        bins: bins,
    });
}
...
function collectRequiredBins(entries, targetPlatform) {
    ...
    for (var _i = 0, entries_1 = entries; _i < entries_1.length; _i++) {
        ...
        var os = (_b = (_a = entry.metadata) === null || _a === void 0 ? void 0 : _a.os) !== null && _b !== void 0 ? _b : [];
        if (os.length > 0 && !os.includes(targetPlatform))
            continue;
        var required = (_e = (_d = (_c = entry.metadata) === null || _c === void 0 ? void 0 : _c.requires) === null || _d === void 0 ? void 0 : _d.bins) !== null && _e !== void 0 ? _e : [];
        var anyBins = (_h = (_g = (_f = entry.metadata) === null || _f === void 0 ? void 0 : _f.requires) === null || _g === void 0 ? void 0 : _g.anyBins) !== null && _h !== void 0 ? _h : [];
...
function buildBinProbeScript(bins) {
    var escaped = bins.map(function (bin) { return "'".concat(bin.replace(/'/g, "'\\''"), "'"); }).join(" ");
    return "for b in ".concat(escaped, "; do if command -v \"$b\" >/dev/null 2>&1; then echo \"$b\"; fi; done");
}
function parseBinProbePayload(payloadJSON, payload) {
    ...
    try {
        ...
        if (Array.isArray(parsed.bins)) {
            return parsed.bins.map(function (bin) { return String(bin).trim(); }).filter(Boolean);
        }
...
function refreshRemoteNodeBins(params) {
    return __awaiter(this, void 0, void 0, function () {
        var canWhich, canRun, workspaceDirs, requiredBins, _i, workspaceDirs_1, workspaceDir, entries, _a, _b, bin, binsList, res, bins, existingBins, nextBins, hasChanged, err_2;
        var _c, _d, _e, _f, _g;
            ...
            switch (_h.label) {
                case 0:
                    ...
                    for (_i = 0, workspaceDirs_1 = workspaceDirs; _i < workspaceDirs_1.length; _i++) {
                        ...
                        entries = (0, skills_js_1.loadWorkspaceSkillEntries)(workspaceDir, { config: params.cfg });
                ...
                case 1:
                    ...
                    return [4 /*yield*/, remoteRegistry.invoke(canWhich
                            ? {
                ...
                case 2:
                    res = _h.sent();
                    if (!res.ok) {
                        logRemoteBinProbeFailure(params.nodeId, (_f = (_e = res.error) === null || _e === void 0 ? void 0 : _e.message) !== null && _f !== void 0 ? _f : "unknown");
                        return [2 /*return*/];
                    }
                    bins = parseBinProbePayload(res.payloadJSON, res.payload);
                    existingBins = (_g = remoteNodes.get(params.nodeId)) === null || _g === void 0 ? void 0 : _g.bins;
                    nextBins = new Set(bins);
                    hasChanged = !areBinSetsEqual(existingBins, nextBins);
                    recordRemoteNodeBins(params.nodeId, bins);
                    if (!hasChanged)
                        return [2 /*return*/];
                    return [4 /*yield*/, (0, node_pairing_js_1.updatePairedNodeMetadata)(params.nodeId, { bins: bins })];
                case 3:
                    _h.sent();
                    (0, refresh_js_1.bumpSkillsSnapshotVersion)({ reason: "remote-node" });
                    return [3 /*break*/, 5];
...
function getRemoteSkillEligibility() {
    var macNodes = __spreadArray([], remoteNodes.values(), true).filter(function (node) { return isMacPlatform(node.platform, node.deviceFamily) && supportsSystemRun(node.commands); });
    ...
    var labels = macNodes.map(function (node) { var _a; return (_a = node.displayName) !== null && _a !== void 0 ? _a : node.nodeId; }).filter(Boolean);
    var note = labels.length > 0
        ? "Remote macOS node available (".concat(labels.join(", "), "). Run macOS-only skills via nodes.run on that node.")
        : "Remote macOS node available. Run macOS-only skills via nodes.run on that node.";
    return {
        platforms: ["darwin"],
        hasBin: function (bin) { return bins.has(bin); },
        hasAnyBin: function (required) { return required.some(function (bin) { return bins.has(bin); }); },
        note: note,
    };
}
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    _a.label = 1;
                case 1:
                    if (!(_i < connected_1.length)) return [3 /*break*/, 4];
                    node = connected_1[_i];
                    return [4 /*yield*/, refreshRemoteNodeBins({
                            nodeId: node.nodeId,
                            platform: node.platform,
                            deviceFamily: node.deviceFamily,
                            commands: node.commands,
