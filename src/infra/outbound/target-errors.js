...
function ambiguousTargetMessage(provider, raw, hint) {
    return "Ambiguous target \"".concat(raw, "\" for ").concat(provider, ". Provide a unique name or an explicit id.").concat(formatTargetHint(hint, true));
}
...
function unknownTargetMessage(provider, raw, hint) {
    return "Unknown target \"".concat(raw, "\" for ").concat(provider, ".").concat(formatTargetHint(hint, true));
}
function unknownTargetError(provider, raw, hint) {
    return new Error(unknownTargetMessage(provider, raw, hint));
}
function formatTargetHint(hint, withLabel) {
    if (withLabel === void 0) { withLabel = false; }
    if (!hint)
        return "";
