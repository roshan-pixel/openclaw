...
function elide(text, limit) {
    if (limit === void 0) { limit = 400; }
    if (!text)
        return text;
    if (text.length <= limit)
        return text;
    return "".concat(text.slice(0, limit), "\u2026 (truncated ").concat(text.length - limit, " chars)");
}
function isLikelyWhatsAppCryptoError(reason) {
    ...
    var raw = reason instanceof Error ? "".concat(reason.message, "\n").concat((_a = reason.stack) !== null && _a !== void 0 ? _a : "") : formatReason(reason);
    var haystack = raw.toLowerCase();
    var hasAuthError = haystack.includes("unsupported state or unable to authenticate data") ||
        haystack.includes("bad mac");
    if (!hasAuthError)
        return false;
    return (haystack.includes("@whiskeysockets/baileys") ||
