...
function findJobOrThrow(state, id) {
    var _a;
    var job = (_a = state.store) === null || _a === void 0 ? void 0 : _a.jobs.find(function (j) { return j.id === id; });
    if (!job)
...
function nextWakeAtMs(state) {
    var _a, _b;
    var jobs = (_b = (_a = state.store) === null || _a === void 0 ? void 0 : _a.jobs) !== null && _b !== void 0 ? _b : [];
    var enabled = jobs.filter(function (j) { return j.enabled && typeof j.state.nextRunAtMs === "number"; });
    if (enabled.length === 0)
        return undefined;
    return enabled.reduce(function (min, j) { return Math.min(min, j.state.nextRunAtMs); }, enabled[0].state.nextRunAtMs);
}
