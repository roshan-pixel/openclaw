...
function requireRef(value) {
    ...
    var ref = roleRef !== null && roleRef !== void 0 ? roleRef : (raw.startsWith("@") ? raw.slice(1) : raw);
    if (!ref)
        throw new Error("ref is required");
    return ref;
}
function normalizeTimeoutMs(timeoutMs, fallback) {
    return Math.max(500, Math.min(120000, timeoutMs !== null && timeoutMs !== void 0 ? timeoutMs : fallback));
}
function toAIFriendlyError(error, selector) {
    ...
    if (message.includes("strict mode violation")) {
        ...
        return new Error("Selector \"".concat(selector, "\" matched ").concat(count, " elements. ") +
    ...
    if ((message.includes("Timeout") || message.includes("waiting for")) &&
        ...
        return new Error("Element \"".concat(selector, "\" not found or not visible. ") +
            "Run a new snapshot to see current page elements.");
    }
    if (message.includes("intercepts pointer events") ||
        message.includes("not visible") ||
        message.includes("not receive pointer events")) {
        return new Error("Element \"".concat(selector, "\" is not interactable (hidden or covered). ") +
