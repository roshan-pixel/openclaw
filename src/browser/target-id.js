...
function resolveTargetIdFromTabs(input, tabs) {
    ...
    var exact = tabs.find(function (t) { return t.targetId === needle; });
    if (exact)
        return { ok: true, targetId: exact.targetId };
    var lower = needle.toLowerCase();
    var matches = tabs.map(function (t) { return t.targetId; }).filter(function (id) { return id.toLowerCase().startsWith(lower); });
