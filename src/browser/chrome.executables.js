...
function execText(command, args, timeoutMs, maxBuffer) {
    ...
    if (maxBuffer === void 0) { maxBuffer = 1024 * 1024; }
    try {
...
function detectDefaultBrowserBundleIdMac() {
    var _a;
    var plistPath = node_path_1.default.join(node_os_1.default.homedir(), "Library/Preferences/com.apple.LaunchServices/com.apple.launchservices.secure.plist");
    if (!exists(plistPath))
        return null;
    var handlersRaw = execText("/usr/bin/plutil", ["-extract", "LSHandlers", "json", "-o", "-", "--", plistPath], 2000, 5 * 1024 * 1024);
    if (!handlersRaw)
        return null;
    var handlers;
    try {
    ...
    var resolveScheme = function (scheme) {
        ...
        for (var _i = 0, handlers_1 = handlers; _i < handlers_1.length; _i++) {
            ...
            if (record.LSHandlerURLScheme !== scheme)
                continue;
            var role = (typeof record.LSHandlerRoleAll === "string" && record.LSHandlerRoleAll) ||
...
function detectDefaultChromiumExecutableLinux() {
    var desktopId = execText("xdg-settings", ["get", "default-web-browser"]) ||
...
function detectDefaultChromiumExecutableWindows() {
    ...
    var command = (progId ? readWindowsCommandForProgId(progId) : null) || readWindowsCommandForProgId("http");
    if (!command)
        return null;
...
function findDesktopFilePath(desktopId) {
    var candidates = [
        node_path_1.default.join(node_os_1.default.homedir(), ".local", "share", "applications", desktopId),
...
function expandWindowsEnvVars(value) {
    return value.replace(/%([^%]+)%/g, function (_match, name) {
        ...
        return key ? ((_a = process.env[key]) !== null && _a !== void 0 ? _a : "%".concat(key, "%")) : _match;
    });
}
...
function findChromeExecutableMac() {
    var candidates = [
        ...
        {
            kind: "chrome",
            path: node_path_1.default.join(node_os_1.default.homedir(), "Applications/Google Chrome.app/Contents/MacOS/Google Chrome"),
        },
        {
            kind: "brave",
        ...
        {
            kind: "brave",
            path: node_path_1.default.join(node_os_1.default.homedir(), "Applications/Brave Browser.app/Contents/MacOS/Brave Browser"),
        },
        {
            kind: "edge",
        ...
        {
            kind: "edge",
            path: node_path_1.default.join(node_os_1.default.homedir(), "Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge"),
        },
        {
            kind: "chromium",
        ...
        {
            kind: "chromium",
            path: node_path_1.default.join(node_os_1.default.homedir(), "Applications/Chromium.app/Contents/MacOS/Chromium"),
        },
        {
            kind: "canary",
        ...
        {
            kind: "canary",
            path: node_path_1.default.join(node_os_1.default.homedir(), "Applications/Google Chrome Canary.app/Contents/MacOS/Google Chrome Canary"),
        },
    ];
...
function findChromeExecutableWindows() {
    ...
    var programFiles = (_b = process.env.ProgramFiles) !== null && _b !== void 0 ? _b : "C:\\Program Files";
    // Must use bracket notation: variable name contains parentheses
    var programFilesX86 = (_c = process.env["ProgramFiles(x86)"]) !== null && _c !== void 0 ? _c : "C:\\Program Files (x86)";
