...
function getRoleSnapshotStats(snapshot, refs) {
    var interactive = Object.values(refs).filter(function (r) { return INTERACTIVE_ROLES.has(r.role); }).length;
    return {
    ...
    return {
        ...
        getDuplicateKeys: function () {
            ...
            for (var _i = 0, refsByKey_1 = refsByKey; _i < refsByKey_1.length; _i++) {
                var _a = refsByKey_1[_i], key = _a[0], refs = _a[1];
                if (refs.length > 1)
                    out.add(key);
            }
            return out;
...
function removeNthFromNonDuplicates(refs, tracker) {
    ...
    for (var _i = 0, _b = Object.entries(refs); _i < _b.length; _i++) {
        var _c = _b[_i], ref = _c[0], data = _c[1];
...
function processLine(line, refs, options, tracker, nextRef) {
    ...
    var prefix = match[1], roleRaw = match[2], name = match[3], suffix = match[4];
...
function buildRoleSnapshotFromAriaSnapshot(ariaSnapshot, options) {
    ...
    if (options.interactive) {
        ...
        for (var _i = 0, lines_1 = lines; _i < lines_1.length; _i++) {
            ...
            if (!match)
                continue;
            var roleRaw = match[2], name_1 = match[3], suffix = match[4];
            if (roleRaw.startsWith("/"))
                continue;
            var role = roleRaw.toLowerCase();
...
function buildRoleSnapshotFromAiSnapshot(aiSnapshot, options) {
    ...
    if (options.interactive) {
        ...
        for (var _i = 0, lines_3 = lines; _i < lines_3.length; _i++) {
            ...
            if (!match)
                continue;
            var roleRaw = match[2], name_2 = match[3], suffix = match[4];
            if (roleRaw.startsWith("/"))
                continue;
            var role = roleRaw.toLowerCase();
            if (!INTERACTIVE_ROLES.has(role))
            ...
            out_1.push("- ".concat(roleRaw).concat(name_2 ? " \"".concat(name_2, "\"") : "").concat(suffix));
        }
        return {
            snapshot: out_1.join("\n") || "(no interactive elements)",
            refs: refs,
    ...
    for (var _a = 0, lines_4 = lines; _a < lines_4.length; _a++) {
        ...
        var roleRaw = match[2], name_3 = match[3], suffix = match[4];
        if (roleRaw.startsWith("/")) {
            out.push(line);
            continue;
        }
