...
function resolveDiscordSystemLocation(params) {
    var isDirectMessage = params.isDirectMessage, isGroupDm = params.isGroupDm, guild = params.guild, channelName = params.channelName;
    if (isDirectMessage)
        return "DM";
    if (isGroupDm)
        return "Group DM #".concat(channelName);
    return (guild === null || guild === void 0 ? void 0 : guild.name) ? "".concat(guild.name, " #").concat(channelName) : "#".concat(channelName);
}
