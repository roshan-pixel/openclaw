...
function normalizeDiscordAllowList(raw, prefixes) {
    ...
    var ids = new Set();
    var names = new Set();
    var allowAll = raw.some(function (entry) { return String(entry).trim() === "*"; });
    var _loop_1 = function (entry) {
        ...
        var prefix = prefixes.find(function (entry) { return text.startsWith(entry); });
        if (prefix) {
...
function resolveDiscordAllowListMatch(params) {
    var allowList = params.allowList, candidate = params.candidate;
...
function buildDiscordChannelKeys(params) {
    ...
    return (0, channel_config_js_1.buildChannelKeyCandidates)(params.id, allowNameMatch ? params.slug : undefined, allowNameMatch ? params.name : undefined);
}
...
function resolveDiscordChannelConfig(params) {
    var guildInfo = params.guildInfo, channelId = params.channelId, channelName = params.channelName, channelSlug = params.channelSlug;
    ...
    var resolved = (0, channel_config_js_1.resolveChannelMatchConfig)(match, resolveDiscordChannelConfigEntry);
    return resolved !== null && resolved !== void 0 ? resolved : { allowed: false };
}
function resolveDiscordChannelConfigWithFallback(params) {
    var _a;
    var guildInfo = params.guildInfo, channelId = params.channelId, channelName = params.channelName, channelSlug = params.channelSlug, parentId = params.parentId, parentName = params.parentName, parentSlug = params.parentSlug, scope = params.scope;
    var channels = guildInfo === null || guildInfo === void 0 ? void 0 : guildInfo.channels;
    if (!channels)
        return null;
    var resolvedParentSlug = parentSlug !== null && parentSlug !== void 0 ? parentSlug : (parentName ? normalizeDiscordSlug(parentName) : "");
    var match = resolveDiscordChannelEntryMatch(channels, {
        id: channelId,
        name: channelName,
        slug: channelSlug,
        ...
        : undefined);
    return (_a = (0, channel_config_js_1.resolveChannelMatchConfig)(match, resolveDiscordChannelConfigEntry)) !== null && _a !== void 0 ? _a : { allowed: false };
}
function resolveDiscordShouldRequireMention(params) {
    ...
    var isBotThread = (_a = params.isAutoThreadOwnedByBot) !== null && _a !== void 0 ? _a : isDiscordAutoThreadOwnedByBot(params);
    if (isBotThread)
        return false;
    return (_e = (_c = (_b = params.channelConfig) === null || _b === void 0 ? void 0 : _b.requireMention) !== null && _c !== void 0 ? _c : (_d = params.guildInfo) === null || _d === void 0 ? void 0 : _d.requireMention) !== null && _e !== void 0 ? _e : true;
}
...
function isDiscordGroupAllowedByPolicy(params) {
    var groupPolicy = params.groupPolicy, guildAllowlisted = params.guildAllowlisted, channelAllowlistConfigured = params.channelAllowlistConfigured, channelAllowed = params.channelAllowed;
...
function resolveGroupDmAllow(params) {
    var channels = params.channels, channelId = params.channelId, channelName = params.channelName, channelSlug = params.channelSlug;
    if (!channels || channels.length === 0)
        return true;
    var allowList = channels.map(function (entry) { return normalizeDiscordSlug(String(entry)); });
    var candidates = [
        normalizeDiscordSlug(channelId),
        channelSlug,
        channelName ? normalizeDiscordSlug(channelName) : "",
    ].filter(Boolean);
    return allowList.includes("*") || candidates.some(function (candidate) { return allowList.includes(candidate); });
}
