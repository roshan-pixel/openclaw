...
function resolveToken(params) {
    ...
    if (!fallback) {
        throw new Error("Discord bot token missing for account \"".concat(params.accountId, "\" (set discord.accounts.").concat(params.accountId, ".token or DISCORD_BOT_TOKEN for default)."));
    }
    return fallback;
}
...
function createDiscordClient(opts, cfg) {
    if (cfg === void 0) { cfg = (0, config_js_1.loadConfig)(); }
...
function parseRecipient(raw) {
    var target = (0, targets_js_1.parseDiscordTarget)(raw, {
        ambiguousMessage: "Ambiguous Discord recipient \"".concat(raw.trim(), "\". Use \"user:").concat(raw.trim(), "\" for DMs or \"channel:").concat(raw.trim(), "\" for channel messages."),
    });
    if (!target) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    accountInfo = (0, accounts_js_1.resolveDiscordAccount)({ cfg: cfg, accountId: accountId });
                    trimmed = raw.trim();
                    parseOptions = {
                        ambiguousMessage: "Ambiguous Discord recipient \"".concat(trimmed, "\". Use \"user:").concat(trimmed, "\" for DMs or \"channel:").concat(trimmed, "\" for channel messages."),
                    };
                    return [4 /*yield*/, (0, targets_js_1.resolveDiscordTarget)(raw, {
                            cfg: cfg,
...
function normalizeStickerIds(raw) {
    var ids = raw.map(function (entry) { return entry.trim(); }).filter(Boolean);
    if (ids.length === 0) {
...
function normalizeDiscordPollInput(input) {
    ...
    return {
        ...
        answers: poll.options.map(function (answer) { return ({ poll_media: { text: answer } }); }),
        duration: duration,
...
function getDiscordErrorCode(err) {
    if (!err || typeof err !== "object")
        return undefined;
    var candidate = "code" in err && err.code !== undefined
        ? err.code
            ...
            switch (_b.label) {
                case 0:
                    ...
                    if (code === DISCORD_CANNOT_DM) {
                        return [2 /*return*/, new send_types_js_1.DiscordSendError("discord dm failed: user blocks dms or privacy settings disallow it", { kind: "dm-blocked" })];
                    }
                ...
                case 1:
                    _b.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, (0, send_permissions_js_1.fetchChannelPermissionsDiscord)(ctx.channelId, {
                            rest: ctx.rest,
                            token: ctx.token,
                        })];
                case 2:
                    ...
                    missing = required.filter(function (permission) { return !current_1.has(permission); });
                    return [3 /*break*/, 4];
                case 3:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 4:
                    missingLabel = missing.length
                        ? "missing permissions in channel ".concat(ctx.channelId, ": ").concat(missing.join(", "))
                        : "missing permissions in channel ".concat(ctx.channelId);
                    return [2 /*return*/, new send_types_js_1.DiscordSendError("".concat(missingLabel, ". bot might be muted or blocked by role/channel overrides"), {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, request(function () {
...
function sendDiscordText(rest, channelId, text, replyTo, request, maxLinesPerMessage, embeds, chunkMode) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    messageReference = replyTo ? { message_id: replyTo, fail_if_not_exists: false } : undefined;
                    ...
                    if (!chunks.length && text)
                        chunks.push(text);
                    if (!(chunks.length === 1)) return [3 /*break*/, 2];
                    return [4 /*yield*/, request(function () {
                            return rest.post(v10_2.Routes.channelMessages(channelId), {
                                body: __assign({ content: chunks[0], message_reference: messageReference }, ((embeds === null || embeds === void 0 ? void 0 : embeds.length) ? { embeds: embeds } : {})),
                            });
                        }, "text")];
                case 1:
                    res = (_a.sent());
                ...
                case 2:
                    ...
                    _loop_1 = function (chunk) {
                        return __generator(this, function (_b) {
                            switch (_b.label) {
                                case 0: return [4 /*yield*/, request(function () {
                                        return rest.post(v10_2.Routes.channelMessages(channelId), {
                                            body: __assign({ content: chunk, message_reference: isFirst ? messageReference : undefined }, (isFirst && (embeds === null || embeds === void 0 ? void 0 : embeds.length) ? { embeds: embeds } : {})),
                                        });
                                    }, "text")];
                                case 1:
                                    last = (_b.sent());
...
function sendDiscordMedia(rest, channelId, text, mediaUrl, replyTo, request, maxLinesPerMessage, embeds, chunkMode) {
    return __awaiter(this, void 0, void 0, function () {
        var media, chunks, caption, messageReference, res, _i, _a, chunk;
        var _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0: return [4 /*yield*/, (0, media_js_1.loadWebMedia)(mediaUrl)];
                case 1:
                    media = _c.sent();
                    chunks = text
                    ...
                    return [4 /*yield*/, request(function () {
                            var _a;
                            return rest.post(v10_2.Routes.channelMessages(channelId), {
                                body: __assign(__assign({ content: caption || undefined, message_reference: messageReference }, ((embeds === null || embeds === void 0 ? void 0 : embeds.length) ? { embeds: embeds } : {})), { files: [
                                        {
                                            data: media.buffer,
                ...
                case 3:
                    ...
                    if (!chunk.trim())
                        return [3 /*break*/, 5];
                    return [4 /*yield*/, sendDiscordText(rest, channelId, chunk, undefined, request, maxLinesPerMessage, undefined, chunkMode)];
                case 4:
                    _c.sent();
                    _c.label = 5;
                case 5:
