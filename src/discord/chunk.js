...
function chunkDiscordText(text, opts) {
    var _a, _b;
    if (opts === void 0) { opts = {}; }
    var maxChars = Math.max(1, Math.floor((_a = opts.maxChars) !== null && _a !== void 0 ? _a : DEFAULT_MAX_CHARS));
    var maxLines = Math.max(1, Math.floor((_b = opts.maxLines) !== null && _b !== void 0 ? _b : DEFAULT_MAX_LINES));
...
function chunkDiscordTextWithMode(text, opts) {
    ...
    var lineChunks = (0, chunk_js_1.chunkMarkdownTextWithMode)(text, Math.max(1, Math.floor((_b = opts.maxChars) !== null && _b !== void 0 ? _b : DEFAULT_MAX_CHARS)), "newline");
    var chunks = [];
...
function rebalanceReasoningItalics(source, chunks) {
    if (chunks.length <= 1)
        return chunks;
    var opensWithReasoningItalics = source.startsWith("Reasoning:\n_") && source.trimEnd().endsWith("_");
