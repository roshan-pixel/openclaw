...
function isInsideFence(fenceSpans, offset) {
    return fenceSpans.some(function (span) { return offset >= span.start && offset < span.end; });
}
function splitMediaFromOutput(raw) {
    ...
    for (var _i = 0, lines_1 = lines; _i < lines_1.length; _i++) {
        ...
        for (var _b = 0, matches_1 = matches; _b < matches_1.length; _b++) {
            ...
            var looksLikeLocalPath = trimmedPayload.startsWith("/") ||
    ...
    return __assign({ text: cleanedText, mediaUrls: media, mediaUrl: media[0] }, (hasAudioAsVoice ? { audioAsVoice: true } : {}));
}
