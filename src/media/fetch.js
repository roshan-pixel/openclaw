...
function readErrorBodySnippet(res_1) {
    return __awaiter(this, arguments, void 0, function (res, maxChars) {
        ...
        if (maxChars === void 0) { maxChars = 200; }
...
function fetchRemoteMedia(options) {
    return __awaiter(this, void 0, void 0, function () {
        var url, fetchImpl, filePathHint, maxBytes, fetcher, res, err_1, statusText, redirected, detail, snippet, contentLength, length_1, buffer, _a, _b, _c, fileNameFromUrl, parsed, base, headerFileName, fileName, filePathForMime, contentType, ext;
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    url = options.url, fetchImpl = options.fetchImpl, filePathHint = options.filePathHint, maxBytes = options.maxBytes;
                ...
                case 3:
                    err_1 = _d.sent();
                    throw new MediaFetchError("fetch_failed", "Failed to fetch media from ".concat(url, ": ").concat(String(err_1)));
                case 4:
                    ...
                    if (!!res.body) return [3 /*break*/, 5];
                    detail = "HTTP ".concat(res.status).concat(statusText, "; empty response body");
                    return [3 /*break*/, 7];
                case 5: return [4 /*yield*/, readErrorBodySnippet(res)];
                case 6:
                    snippet = _d.sent();
                    if (snippet)
                        detail += "; body: ".concat(snippet);
                    _d.label = 7;
                case 7: throw new MediaFetchError("http_error", "Failed to fetch media from ".concat(url).concat(redirected, ": ").concat(detail));
                case 8:
                    contentLength = res.headers.get("content-length");
                    if (maxBytes && contentLength) {
                        length_1 = Number(contentLength);
                        if (Number.isFinite(length_1) && length_1 > maxBytes) {
                            throw new MediaFetchError("max_bytes", "Failed to fetch media from ".concat(url, ": content length ").concat(length_1, " exceeds maxBytes ").concat(maxBytes));
                        }
                    }
                    if (!maxBytes) return [3 /*break*/, 10];
                ...
                case 12:
                    ...
                    fileName = headerFileName || fileNameFromUrl || (filePathHint ? node_path_1.default.basename(filePathHint) : undefined);
                    filePathForMime = headerFileName && node_path_1.default.extname(headerFileName) ? headerFileName : (filePathHint !== null && filePathHint !== void 0 ? filePathHint : url);
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    body = res.body;
                    if (!(!body || typeof body.getReader !== "function")) return [3 /*break*/, 2];
                    _b = (_a = Buffer).from;
                    return [4 /*yield*/, res.arrayBuffer()];
                case 1:
                    fallback = _b.apply(_a, [_e.sent()]);
                    if (fallback.length > maxBytes) {
                        throw new MediaFetchError("max_bytes", "Failed to fetch media from ".concat(res.url || "response", ": payload exceeds maxBytes ").concat(maxBytes));
                    }
                ...
                case 5:
                    ...
                    if (done)
                        return [3 /*break*/, 12];
                    if (!(value === null || value === void 0 ? void 0 : value.length)) return [3 /*break*/, 11];
                    total += value.length;
                ...
                case 8:
                    _d = _e.sent();
                    return [3 /*break*/, 9];
                case 9: throw new MediaFetchError("max_bytes", "Failed to fetch media from ".concat(res.url || "response", ": payload exceeds maxBytes ").concat(maxBytes));
                case 10:
                    chunks.push(value);
                    _e.label = 11;
                ...
                case 13:
                    ...
                    return [7 /*endfinally*/];
                case 14: return [2 /*return*/, Buffer.concat(chunks.map(function (chunk) { return Buffer.from(chunk); }), total)];
            }
        });
    });
}
