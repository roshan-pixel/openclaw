...
function createSlackMessageHandler(params) {
    ...
    var debounceMs = (0, inbound_debounce_js_1.resolveInboundDebounceMs)({ cfg: ctx.cfg, channel: "slack" });
    var threadTsResolver = (0, thread_resolution_js_1.createSlackThreadTsResolver)({ client: ctx.app.client });
    var debouncer = (0, inbound_debounce_js_1.createInboundDebouncer)({
        debounceMs: debounceMs,
        buildKey: function (entry) {
            var _a, _b;
            var senderId = (_a = entry.message.user) !== null && _a !== void 0 ? _a : entry.message.bot_id;
            if (!senderId)
                return null;
            var messageTs = (_b = entry.message.ts) !== null && _b !== void 0 ? _b : entry.message.event_ts;
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        if (!last)
                            return [2 /*return*/];
                        combinedText = entries.length === 1
                            ? ((_a = last.message.text) !== null && _a !== void 0 ? _a : "")
                            : entries
                                .map(function (entry) { var _a; return (_a = entry.message.text) !== null && _a !== void 0 ? _a : ""; })
                                .filter(Boolean)
                                .join("\n");
                        combinedMentioned = entries.some(function (entry) { return Boolean(entry.opts.wasMentioned); });
                        syntheticMessage = __assign(__assign({}, last.message), { text: combinedText });
                        return [4 /*yield*/, (0, prepare_js_1.prepareSlackMessage)({
                                ctx: ctx,
                                account: account,
                                message: syntheticMessage,
                                opts: __assign(__assign({}, last.opts), { wasMentioned: combinedMentioned || last.opts.wasMentioned }),
                            })];
                    case 1:
                        prepared = _b.sent();
                        if (!prepared)
                            return [2 /*return*/];
                        if (entries.length > 1) {
                            ids = entries.map(function (entry) { return entry.message.ts; }).filter(Boolean);
                            if (ids.length > 0) {
        ...
        onError: function (err) {
            var _a, _b;
            (_b = (_a = ctx.runtime).error) === null || _b === void 0 ? void 0 : _b.call(_a, "slack inbound debounce flush failed: ".concat(String(err)));
        },
    });
    return function (message, opts) { return __awaiter(_this, void 0, void 0, function () {
        var resolvedMessage;
            ...
            switch (_a.label) {
                case 0:
                    ...
                    if (ctx.markMessageSeen(message.channel, message.ts))
                        return [2 /*return*/];
                    return [4 /*yield*/, threadTsResolver.resolve({ message: message, source: opts.source })];
                case 1:
