...
function escapeSlackMrkdwnContent(text) {
    ...
    var out = [];
    var lastIndex = 0;
    for (var match = SLACK_ANGLE_TOKEN_RE.exec(text); match; match = SLACK_ANGLE_TOKEN_RE.exec(text)) {
...
function buildSlackLink(link, text) {
    ...
    var useMarkup = trimmedLabel.length > 0 && trimmedLabel !== href && trimmedLabel !== comparableHref;
    if (!useMarkup)
        return null;
