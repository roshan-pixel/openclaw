...
function mergeStyles(styles) {
    var sorted = __spreadArray([], styles, true).sort(function (a, b) {
        if (a.start !== b.start)
...
function applyInsertionsToStyles(spans, insertions) {
    if (insertions.length === 0)
        return spans;
    var sortedInsertions = __spreadArray([], insertions, true).sort(function (a, b) { return a.pos - b.pos; });
    var updated = spans;
...
function renderSignalText(ir) {
    ...
    if (!text)
        return { text: "", styles: [] };
    var sortedLinks = __spreadArray([], ir.links, true).sort(function (a, b) { return a.start - b.start; });
    var out = "";
    var cursor = 0;
    var insertions = [];
    ...
    var clamped = clampStyles(adjusted.map(function (span) { return ({
        start: span.start,
...
function markdownToSignalTextChunks(markdown, limit, options) {
    ...
    return chunks.map(function (chunk) { return renderSignalText(chunk); });
}
