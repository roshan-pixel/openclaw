...
function isSignalSenderAllowed(sender, allowFrom) {
    if (allowFrom.length === 0)
        return false;
    var parsed = allowFrom
        .map(parseSignalAllowEntry)
        .filter(function (entry) { return entry !== null; });
    if (parsed.some(function (entry) { return entry.kind === "any"; }))
        return true;
...
function isSignalGroupAllowed(params) {
    var groupPolicy = params.groupPolicy, allowFrom = params.allowFrom, sender = params.sender;
