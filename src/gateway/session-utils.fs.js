...
function readSessionMessages(sessionId, storePath, sessionFile) {
    ...
    var filePath = candidates.find(function (p) { return node_fs_1.default.existsSync(p); });
    if (!filePath)
        return [];
...
function resolveSessionTranscriptCandidates(sessionId, storePath, sessionFile, agentId) {
    ...
    candidates.push(node_path_1.default.join(home, ".openclaw", "sessions", "".concat(sessionId, ".jsonl")));
    return candidates;
}
...
function capArrayByJsonBytes(items, maxBytes) {
    ...
    var parts = items.map(function (item) { return jsonUtf8Bytes(item); });
    var bytes = 2 + parts.reduce(function (a, b) { return a + b; }, 0) + (items.length - 1);
    var start = 0;
    ...
    try {
        ...
        for (var i = tailLines.length - 1; i >= 0; i--) {
            ...
            try {
                var parsed = JSON.parse(line);
                var msg = parsed === null || parsed === void 0 ? void 0 : parsed.message;
                if ((msg === null || msg === void 0 ? void 0 : msg.role) === "user" || (msg === null || msg === void 0 ? void 0 : msg.role) === "assistant") {
                    var text = extractTextFromContent(msg.content);
                    if (text)
                        return text;
                }
...
function extractPreviewText(message) {
    ...
    if (Array.isArray(message.content)) {
        var parts = message.content
            .map(function (entry) { return (typeof (entry === null || entry === void 0 ? void 0 : entry.text) === "string" ? entry.text : ""); })
...
function isToolCall(message) {
    ...
    return message.content.some(function (entry) {
        if (entry === null || entry === void 0 ? void 0 : entry.name)
            return true;
        var raw = typeof (entry === null || entry === void 0 ? void 0 : entry.type) === "string" ? entry.type.toLowerCase() : "";
...
function extractToolNames(message) {
    ...
    if (Array.isArray(message.content)) {
        for (var _i = 0, _a = message.content; _i < _a.length; _i++) {
            var entry = _a[_i];
            if (typeof (entry === null || entry === void 0 ? void 0 : entry.name) === "string" && entry.name.trim()) {
...
function extractMediaSummary(message) {
    ...
    for (var _i = 0, _a = message.content; _i < _a.length; _i++) {
        var entry = _a[_i];
        var raw = typeof (entry === null || entry === void 0 ? void 0 : entry.type) === "string" ? entry.type.trim().toLowerCase() : "";
...
function readSessionPreviewItemsFromTranscript(sessionId, storePath, sessionFile, agentId, maxItems, maxChars) {
    ...
    for (var _i = 0, PREVIEW_READ_SIZES_1 = PREVIEW_READ_SIZES; _i < PREVIEW_READ_SIZES_1.length; _i++) {
