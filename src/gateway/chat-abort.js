...
function resolveChatRunExpiresAtMs(params) {
    var now = params.now, timeoutMs = params.timeoutMs, _a = params.graceMs, graceMs = _a === void 0 ? 60000 : _a, _b = params.minMs, minMs = _b === void 0 ? 2 * 60000 : _b, _c = params.maxMs, maxMs = _c === void 0 ? 24 * 60 * 60000 : _c;
...
function broadcastChatAborted(ops, params) {
    var _a;
    var runId = params.runId, sessionKey = params.sessionKey, stopReason = params.stopReason;
    var payload = {
        runId: runId,
...
function abortChatRunsForSessionKey(ops, params) {
    var sessionKey = params.sessionKey, stopReason = params.stopReason;
    var runIds = [];
    for (var _i = 0, _a = ops.chatAbortControllers; _i < _a.length; _i++) {
        var _b = _a[_i], runId = _b[0], active = _b[1];
        if (active.sessionKey !== sessionKey)
            continue;
        var res = abortChatRunById(ops, { runId: runId, sessionKey: sessionKey, stopReason: stopReason });
        if (res.aborted)
            runIds.push(runId);
    }
