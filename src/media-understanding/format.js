...
function extractMediaUserText(body) {
    var _a;
    var trimmed = (_a = body === null || body === void 0 ? void 0 : body.trim()) !== null && _a !== void 0 ? _a : "";
    if (!trimmed)
        return undefined;
...
function formatMediaUnderstandingBody(params) {
    var _a, _b, _c, _d;
    var outputs = params.outputs.filter(function (output) { return output.text.trim(); });
    if (outputs.length === 0) {
    ...
    for (var _i = 0, outputs_1 = outputs; _i < outputs_1.length; _i++) {
        ...
        counts.set(output.kind, ((_b = counts.get(output.kind)) !== null && _b !== void 0 ? _b : 0) + 1);
    }
    var seen = new Map();
    for (var _e = 0, outputs_2 = outputs; _e < outputs_2.length; _e++) {
        ...
        if (output.kind === "audio.transcription") {
            sections.push(formatSection("Audio".concat(suffix), "Transcript", output.text, outputs.length === 1 ? userText : undefined));
            continue;
        }
        if (output.kind === "image.description") {
            sections.push(formatSection("Image".concat(suffix), "Description", output.text, outputs.length === 1 ? userText : undefined));
            continue;
        }
        sections.push(formatSection("Video".concat(suffix), "Description", output.text, outputs.length === 1 ? userText : undefined));
    }
...
function formatAudioTranscripts(outputs) {
    ...
    return outputs.map(function (output, index) { return "Audio ".concat(index + 1, ":\n").concat(output.text); }).join("\n\n");
}
