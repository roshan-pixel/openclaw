...
function buildMediaUnderstandingRegistry(overrides) {
    ...
    if (overrides) {
        for (var _b = 0, _c = Object.entries(overrides); _b < _c.length; _b++) {
            var _d = _c[_b], key = _d[0], provider = _d[1];
            var normalizedKey = normalizeMediaProviderId(key);
            var existing = registry.get(normalizedKey);
            var merged = existing
                ? __assign(__assign(__assign({}, existing), provider), { capabilities: (_a = provider.capabilities) !== null && _a !== void 0 ? _a : existing.capabilities }) : provider;
