...
var STYLE_RANK = new Map(STYLE_ORDER.map(function (style, index) { return [style, index]; }));
function sortStyleSpans(spans) {
    return __spreadArray([], spans, true).sort(function (a, b) {
        var _a, _b;
        if (a.start !== b.start)
            return a.start - b.start;
        if (a.end !== b.end)
            return b.end - a.end;
        return ((_a = STYLE_RANK.get(a.style)) !== null && _a !== void 0 ? _a : 0) - ((_b = STYLE_RANK.get(b.style)) !== null && _b !== void 0 ? _b : 0);
    });
}
function renderMarkdownWithMarkers(ir, options) {
    ...
    var styled = sortStyleSpans(ir.styles.filter(function (span) { return Boolean(styleMarkers[span.style]); }));
    ...
    var points = __spreadArray([], boundaries, true).sort(function (a, b) { return a - b; });
    ...
    for (var i = 0; i < points.length; i += 1) {
        ...
        while (stack.length && ((_b = stack[stack.length - 1]) === null || _b === void 0 ? void 0 : _b.end) === pos) {
            var item = stack.pop();
            if (item)
                out += item.close;
        }
        ...
        if (openingLinks && openingLinks.length > 0) {
            for (var _g = 0, _h = openingLinks.entries(); _g < _h.length; _g++) {
                var _j = _h[_g], index = _j[0], link = _j[1];
                openingItems.push({
                    end: link.end,
                    open: link.open,
                    close: link.close,
        ...
        if (openingStyles) {
            for (var _k = 0, _l = openingStyles.entries(); _k < _l.length; _k++) {
                var _m = _l[_k], index = _m[0], span = _m[1];
