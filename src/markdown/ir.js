...
function resolveRenderTarget(state) {
    var _a, _b;
    return (_b = (_a = state.table) === null || _a === void 0 ? void 0 : _a.currentCell) !== null && _b !== void 0 ? _b : state;
}
...
function renderTableAsBullets(state) {
    ...
    var rows = state.table.rows.map(function (row) { return row.map(trimCell); });
...
function renderTableAsCode(state) {
    ...
    var headers = state.table.headers.map(trimCell);
    var rows = state.table.rows.map(function (row) { return row.map(trimCell); });
    var columnCount = Math.max.apply(Math, __spreadArray([headers.length], rows.map(function (row) { return row.length; }), false));
    if (columnCount === 0)
        return;
    var widths = Array.from({ length: columnCount }, function () { return 0; });
    var updateWidths = function (cells) {
        var _a;
        for (var i = 0; i < columnCount; i += 1) {
            var cell = cells[i];
            var width = (_a = cell === null || cell === void 0 ? void 0 : cell.text.length) !== null && _a !== void 0 ? _a : 0;
            if (widths[i] < width)
                widths[i] = width;
        }
    };
    ...
    var appendRow = function (cells) {
        ...
        for (var i = 0; i < columnCount; i += 1) {
            ...
            var pad = widths[i] - ((_a = cell === null || cell === void 0 ? void 0 : cell.text.length) !== null && _a !== void 0 ? _a : 0);
            if (pad > 0)
...
function mergeStyleSpans(spans) {
    var sorted = __spreadArray([], spans, true).sort(function (a, b) {
        if (a.start !== b.start)
...
function chunkMarkdownIR(ir, limit) {
    ...
    chunks.forEach(function (chunk, index) {
        ...
        if (!chunk)
            return;
        if (index > 0) {
            while (cursor < ir.text.length && /\s/.test((_a = ir.text[cursor]) !== null && _a !== void 0 ? _a : "")) {
                cursor += 1;
            }
        }
        var start = cursor;
