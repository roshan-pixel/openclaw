...
function extractMarkdownTables(text) {
    ...
    for (var i = matches.length - 1; i >= 0; i--) {
        var _a = matches[i], fullMatch = _a.fullMatch, table = _a.table;
...
function convertTableToFlexBubble(table) {
    ...
    var headerCells = table.headers.map(function (header) { return parseCell(header); });
    var rowCells = table.rows.map(function (row) { return row.map(function (cell) { return parseCell(cell); }); });
    var hasInlineMarkup = headerCells.some(function (cell) { return cell.hasMarkup; }) ||
        rowCells.some(function (row) { return row.some(function (cell) { return cell.hasMarkup; }); });
    ...
    if (table.headers.length === 2 && !hasInlineMarkup) {
        var items = rowCells.map(function (row) {
            var _a, _b, _c, _d;
            return ({
                name: (_b = (_a = row[0]) === null || _a === void 0 ? void 0 : _a.text) !== null && _b !== void 0 ? _b : "-",
                value: (_d = (_c = row[1]) === null || _c === void 0 ? void 0 : _c.text) !== null && _d !== void 0 ? _d : "-",
            });
        });
        return (0, flex_templates_js_1.createReceiptCard)({
            title: headerCells.map(function (cell) { return cell.text; }).join(" / "),
            items: items,
        });
    }
    ...
    var headerRow = {
        type: "box",
        layout: "horizontal",
        contents: headerCells.map(function (cell) { return ({
            type: "text",
            text: cell.text,
            weight: "bold",
            size: "sm",
    ...
    var dataRows = rowCells.slice(0, 10).map(function (row, rowIndex) {
        var rowContents = table.headers.map(function (_, colIndex) {
            var _a;
            var cell = (_a = row[colIndex]) !== null && _a !== void 0 ? _a : { text: "-", bold: false, hasMarkup: false };
            return {
                type: "text",
                text: cell.text,
                size: "sm",
...
function extractCodeBlocks(text) {
    ...
    for (var i = matches.length - 1; i >= 0; i--) {
        var _a = matches[i], fullMatch = _a.fullMatch, block = _a.block;
...
function convertLinksToFlexBubble(links) {
    var buttons = links.slice(0, 4).map(function (link, index) { return ({
        type: "button",
        action: {
            type: "uri",
...
function processLineMessage(text) {
    ...
    var _a = extractMarkdownTables(processedText), tables = _a.tables, textWithoutTables = _a.textWithoutTables;
    ...
    var _b = extractCodeBlocks(processedText), codeBlocks = _b.codeBlocks, textWithoutCode = _b.textWithoutCode;
