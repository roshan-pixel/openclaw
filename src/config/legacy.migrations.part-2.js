...
exports.LEGACY_CONFIG_MIGRATIONS_PART_2 = [
    {
        ...
        apply: function (raw, changes) {
            ...
            var defaults = (0, legacy_shared_js_1.getRecord)((_a = (0, legacy_shared_js_1.getRecord)(raw.agents)) === null || _a === void 0 ? void 0 : _a.defaults);
            ...
            var legacyImageModel = typeof agent.imageModel === "string" ? String(agent.imageModel) : undefined;
            var legacyAllowed = Array.isArray(agent.allowedModels)
            ...
            var legacyImageModelFallbacks = Array.isArray(agent.imageModelFallbacks)
                ...
                : [];
            var legacyAliases = agent.modelAliases && typeof agent.modelAliases === "object"
                ? agent.modelAliases
                : {};
            var hasLegacy = legacyModel ||
                legacyImageModel ||
            ...
            if (!hasLegacy)
                return;
            var models = agent.models && typeof agent.models === "object"
                ? agent.models
                : {};
            ...
            for (var _b = 0, legacyModelFallbacks_1 = legacyModelFallbacks; _b < legacyModelFallbacks_1.length; _b++) {
                var key = legacyModelFallbacks_1[_b];
                ensureModel(key);
            }
            for (var _c = 0, legacyImageModelFallbacks_1 = legacyImageModelFallbacks; _c < legacyImageModelFallbacks_1.length; _c++) {
            ...
            for (var _f = 0, _g = Object.entries(legacyAliases); _f < _g.length; _f++) {
                var _h = _g[_f], alias = _h[0], targetRaw = _h[1];
                if (typeof targetRaw !== "string")
                    continue;
                var target = targetRaw.trim();
                if (!target)
                    continue;
                var entry = models[target] && typeof models[target] === "object"
                    ? models[target]
                    : {};
                if (!("alias" in entry)) {
                    entry.alias = alias;
            ...
            var currentModel = agent.model && typeof agent.model === "object"
                ? agent.model
                : null;
            if (currentModel) {
            ...
            var currentImageModel = agent.imageModel && typeof agent.imageModel === "object"
                ? agent.imageModel
                : null;
            ...
            if (legacyModel !== undefined) {
                changes.push("Migrated ".concat(label, ".model string \u2192 ").concat(label, ".model.primary."));
            }
            if (legacyModelFallbacks.length > 0) {
                changes.push("Migrated ".concat(label, ".modelFallbacks \u2192 ").concat(label, ".model.fallbacks."));
            }
            if (legacyImageModel !== undefined) {
                changes.push("Migrated ".concat(label, ".imageModel string \u2192 ").concat(label, ".imageModel.primary."));
            }
            if (legacyImageModelFallbacks.length > 0) {
                changes.push("Migrated ".concat(label, ".imageModelFallbacks \u2192 ").concat(label, ".imageModel.fallbacks."));
            }
            ...
            if (Object.keys(legacyAliases).length > 0) {
                changes.push("Migrated ".concat(label, ".modelAliases \u2192 ").concat(label, ".models.*.alias."));
            }
        ...
        apply: function (raw, changes) {
            ...
            if (routingAgents) {
                for (var _i = 0, _a = Object.entries(routingAgents); _i < _a.length; _i++) {
                    var _b = _a[_i], rawId = _b[0], entryRaw = _b[1];
                    ...
                    if ("mentionPatterns" in entryCopy) {
                        ...
                        if (groupChat.mentionPatterns === undefined) {
                            ...
                            changes.push("Moved routing.agents.".concat(agentId, ".mentionPatterns \u2192 agents.list (id \"").concat(agentId, "\").groupChat.mentionPatterns."));
                        }
                        else {
                            changes.push("Removed routing.agents.".concat(agentId, ".mentionPatterns (agents.list groupChat mentionPatterns already set)."));
                        }
                    ...
                    if (legacySandbox) {
                        ...
                        if (sandboxTools) {
                            ...
                            changes.push("Moved routing.agents.".concat(agentId, ".sandbox.tools \u2192 agents.list (id \"").concat(agentId, "\").tools.sandbox.tools."));
                        }
            ...
            var defaultAgentId = typeof routing.defaultAgentId === "string" ? routing.defaultAgentId.trim() : "";
            if (defaultAgentId) {
                var hasDefault = list.some(function (entry) { return (0, legacy_shared_js_1.isRecord)(entry) && entry.default === true; });
                if (!hasDefault) {
                    var entry = (0, legacy_shared_js_1.ensureAgentEntry)(list, defaultAgentId);
                    entry.default = true;
                    changes.push("Moved routing.defaultAgentId \u2192 agents.list (id \"".concat(defaultAgentId, "\").default."));
                }
                else {
