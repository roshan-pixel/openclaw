...
function legacyStateDirs(homedir) {
    ...
    return LEGACY_STATE_DIRNAMES.map(function (dir) { return node_path_1.default.join(homedir(), dir); });
}
...
function resolveStateDir(env, homedir) {
    ...
    if (env === void 0) { env = process.env; }
    if (homedir === void 0) { homedir = node_os_1.default.homedir; }
    var override = ((_a = env.OPENCLAW_STATE_DIR) === null || _a === void 0 ? void 0 : _a.trim()) || ((_b = env.CLAWDBOT_STATE_DIR) === null || _b === void 0 ? void 0 : _b.trim());
    if (override)
...
function resolveCanonicalConfigPath(env, stateDir) {
    var _a, _b;
    if (env === void 0) { env = process.env; }
    if (stateDir === void 0) { stateDir = resolveStateDir(env, node_os_1.default.homedir); }
    var override = ((_a = env.OPENCLAW_CONFIG_PATH) === null || _a === void 0 ? void 0 : _a.trim()) || ((_b = env.CLAWDBOT_CONFIG_PATH) === null || _b === void 0 ? void 0 : _b.trim());
    if (override)
        return resolveUserPath(override);
...
function resolveConfigPath(env, stateDir, homedir) {
    ...
    ], LEGACY_CONFIG_FILENAMES.map(function (name) { return node_path_1.default.join(stateDir, name); }), true);
    var existing = candidates.find(function (candidate) {
        try {
            return node_fs_1.default.existsSync(candidate);
        }
...
function resolveDefaultConfigCandidates(env, homedir) {
    ...
    if (env === void 0) { env = process.env; }
    if (homedir === void 0) { homedir = node_os_1.default.homedir; }
    var explicit = ((_a = env.OPENCLAW_CONFIG_PATH) === null || _a === void 0 ? void 0 : _a.trim()) || ((_b = env.CLAWDBOT_CONFIG_PATH) === null || _b === void 0 ? void 0 : _b.trim());
    if (explicit)
        return [resolveUserPath(explicit)];
    var candidates = [];
    var openclawStateDir = ((_c = env.OPENCLAW_STATE_DIR) === null || _c === void 0 ? void 0 : _c.trim()) || ((_d = env.CLAWDBOT_STATE_DIR) === null || _d === void 0 ? void 0 : _d.trim());
    if (openclawStateDir) {
        var resolved_1 = resolveUserPath(openclawStateDir);
        candidates.push(node_path_1.default.join(resolved_1, CONFIG_FILENAME));
        candidates.push.apply(candidates, LEGACY_CONFIG_FILENAMES.map(function (name) { return node_path_1.default.join(resolved_1, name); }));
    }
    ...
    var _loop_1 = function (dir) {
        ...
        candidates.push.apply(candidates, LEGACY_CONFIG_FILENAMES.map(function (name) { return node_path_1.default.join(dir, name); }));
    };
...
function resolveGatewayLockDir(tmpdir) {
    if (tmpdir === void 0) { tmpdir = node_os_1.default.tmpdir; }
    var base = tmpdir();
...
function resolveGatewayPort(cfg, env) {
    var _a, _b, _c;
    if (env === void 0) { env = process.env; }
    var envRaw = ((_a = env.OPENCLAW_GATEWAY_PORT) === null || _a === void 0 ? void 0 : _a.trim()) || ((_b = env.CLAWDBOT_GATEWAY_PORT) === null || _b === void 0 ? void 0 : _b.trim());
    if (envRaw) {
        var parsed = Number.parseInt(envRaw, 10);
        if (Number.isFinite(parsed) && parsed > 0)
            return parsed;
    }
    var configPort = (_c = cfg === null || cfg === void 0 ? void 0 : cfg.gateway) === null || _c === void 0 ? void 0 : _c.port;
