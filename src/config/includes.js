...
exports.CircularIncludeError = exports.ConfigIncludeError = exports.MAX_INCLUDE_DEPTH = exports.INCLUDE_KEY = void 0;
...
var CircularIncludeError = /** @class */ (function (_super) {
    ...
    function CircularIncludeError(chain) {
        var _this = _super.call(this, "Circular include detected: ".concat(chain.join(" -> ")), chain[chain.length - 1]) || this;
        _this.chain = chain;
...
var IncludeProcessor = /** @class */ (function () {
    ...
    IncludeProcessor.prototype.process = function (obj) {
        ...
        if (Array.isArray(obj)) {
            return obj.map(function (item) { return _this.process(item); });
        }
    ...
    IncludeProcessor.prototype.processInclude = function (obj) {
        ...
        var otherKeys = Object.keys(obj).filter(function (k) { return k !== exports.INCLUDE_KEY; });
        var included = this.resolveInclude(includeValue);
        if (otherKeys.length === 0) {
            return included;
        }
        if (!isPlainObject(included)) {
            throw new ConfigIncludeError("Sibling keys require included content to be an object", typeof includeValue === "string" ? includeValue : exports.INCLUDE_KEY);
        }
        ...
        if (Array.isArray(value)) {
            return value.reduce(function (merged, item) {
                if (typeof item !== "string") {
                    throw new ConfigIncludeError("Invalid $include array item: expected string, got ".concat(typeof item), String(item));
                }
                return deepMerge(merged, _this.loadFile(item));
            }, {});
        }
        throw new ConfigIncludeError("Invalid $include value: expected string or array of strings, got ".concat(typeof value), String(value));
    };
    ...
    IncludeProcessor.prototype.checkCircular = function (resolvedPath) {
        if (this.visited.has(resolvedPath)) {
            throw new CircularIncludeError(__spreadArray(__spreadArray([], this.visited, true), [resolvedPath], false));
        }
    };
    IncludeProcessor.prototype.checkDepth = function (includePath) {
        if (this.depth >= exports.MAX_INCLUDE_DEPTH) {
            throw new ConfigIncludeError("Maximum include depth (".concat(exports.MAX_INCLUDE_DEPTH, ") exceeded at: ").concat(includePath), includePath);
        }
    };
    IncludeProcessor.prototype.readFile = function (includePath, resolvedPath) {
        try {
            ...
            throw new ConfigIncludeError("Failed to read include file: ".concat(includePath, " (resolved: ").concat(resolvedPath, ")"), includePath, err instanceof Error ? err : undefined);
        }
    };
    IncludeProcessor.prototype.parseFile = function (includePath, resolvedPath, raw) {
        try {
            ...
            throw new ConfigIncludeError("Failed to parse include file: ".concat(includePath, " (resolved: ").concat(resolvedPath, ")"), includePath, err instanceof Error ? err : undefined);
        }
    };
    IncludeProcessor.prototype.processNested = function (resolvedPath, parsed) {
        ...
        nested.visited = new Set(__spreadArray(__spreadArray([], this.visited, true), [resolvedPath], false));
...
// ============================================================================
var defaultResolver = {
    readFile: function (p) { return node_fs_1.default.readFileSync(p, "utf-8"); },
    parseJson: function (raw) { return json5_1.default.parse(raw); },
};
/**
