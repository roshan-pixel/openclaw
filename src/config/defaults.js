...
function resolveModelCost(raw) {
    return {
        input: typeof (raw === null || raw === void 0 ? void 0 : raw.input) === "number" ? raw.input : DEFAULT_MODEL_COST.input,
        output: typeof (raw === null || raw === void 0 ? void 0 : raw.output) === "number" ? raw.output : DEFAULT_MODEL_COST.output,
        cacheRead: typeof (raw === null || raw === void 0 ? void 0 : raw.cacheRead) === "number" ? raw.cacheRead : DEFAULT_MODEL_COST.cacheRead,
        cacheWrite: typeof (raw === null || raw === void 0 ? void 0 : raw.cacheWrite) === "number" ? raw.cacheWrite : DEFAULT_MODEL_COST.cacheWrite,
    };
}
function resolveAnthropicDefaultAuthMode(cfg) {
    var _a, _b, _c, _d, _e, _f, _g;
    var profiles = (_b = (_a = cfg.auth) === null || _a === void 0 ? void 0 : _a.profiles) !== null && _b !== void 0 ? _b : {};
    ...
    var order = (_e = (_d = (_c = cfg.auth) === null || _c === void 0 ? void 0 : _c.order) === null || _d === void 0 ? void 0 : _d.anthropic) !== null && _e !== void 0 ? _e : [];
    ...
    var hasOauth = anthropicProfiles.some(function (_a) {
        var profile = _a[1];
        return (profile === null || profile === void 0 ? void 0 : profile.mode) === "oauth" || (profile === null || profile === void 0 ? void 0 : profile.mode) === "token";
    });
...
function applyMessageDefaults(cfg) {
    ...
    var hasAckScope = (messages === null || messages === void 0 ? void 0 : messages.ackReactionScope) !== undefined;
    if (hasAckScope)
        return cfg;
...
function applySessionDefaults(cfg, options) {
    ...
    var next = __assign(__assign({}, cfg), { session: __assign(__assign({}, session), { mainKey: "main" }) });
...
function applyTalkApiKey(config) {
    ...
    if (!resolved)
        return config;
    var existing = (_b = (_a = config.talk) === null || _a === void 0 ? void 0 : _a.apiKey) === null || _b === void 0 ? void 0 : _b.trim();
    if (existing)
        return config;
    return __assign(__assign({}, config), { talk: __assign(__assign({}, config.talk), { apiKey: resolved }) });
}
function applyModelDefaults(cfg) {
    ...
    if (providerConfig) {
        ...
        var _loop_1 = function (providerId, provider) {
            ...
            var nextModels_1 = models.map(function (model) {
                ...
                if (raw.reasoning !== reasoning)
                    modelMutated = true;
                var input = (_a = raw.input) !== null && _a !== void 0 ? _a : __spreadArray([], DEFAULT_MODEL_INPUT, true);
                if (raw.input === undefined)
                    modelMutated = true;
                var cost = resolveModelCost(raw.cost);
                var costMutated = !raw.cost ||
                ...
                return __assign(__assign({}, raw), { reasoning: reasoning, input: input, cost: cost, contextWindow: contextWindow, maxTokens: maxTokens });
            });
            if (!providerMutated)
                return "continue";
        ...
        for (var _i = 0, _d = Object.entries(providerConfig); _i < _d.length; _i++) {
            var _e = _d[_i], providerId = _e[0], provider = _e[1];
            _loop_1(providerId, provider);
        }
        if (mutated) {
            nextCfg = __assign(__assign({}, nextCfg), { models: __assign(__assign({}, nextCfg.models), { providers: nextProviders }) });
        }
    }
    ...
    for (var _f = 0, _g = Object.entries(DEFAULT_MODEL_ALIASES); _f < _g.length; _f++) {
        var _h = _g[_f], alias = _h[0], target = _h[1];
    ...
    if (!mutated)
        return cfg;
    return __assign(__assign({}, nextCfg), { agents: __assign(__assign({}, nextCfg.agents), { defaults: __assign(__assign({}, existingAgent), { models: nextModels }) }) });
}
function applyAgentDefaults(cfg) {
    var _a;
    var agents = cfg.agents;
    var defaults = agents === null || agents === void 0 ? void 0 : agents.defaults;
    var hasMax = typeof (defaults === null || defaults === void 0 ? void 0 : defaults.maxConcurrent) === "number" && Number.isFinite(defaults.maxConcurrent);
    var hasSubMax = typeof ((_a = defaults === null || defaults === void 0 ? void 0 : defaults.subagents) === null || _a === void 0 ? void 0 : _a.maxConcurrent) === "number" &&
    ...
    var nextSubagents = (defaults === null || defaults === void 0 ? void 0 : defaults.subagents) ? __assign({}, defaults.subagents) : {};
    if (!hasSubMax) {
    ...
    if (!mutated)
        return cfg;
    return __assign(__assign({}, cfg), { agents: __assign(__assign({}, agents), { defaults: __assign(__assign({}, nextDefaults), { subagents: nextSubagents }) }) });
}
function applyLoggingDefaults(cfg) {
    ...
    if (logging.redactSensitive)
        return cfg;
    return __assign(__assign({}, cfg), { logging: __assign(__assign({}, logging), { redactSensitive: "tools" }) });
}
function applyContextPruningDefaults(cfg) {
    ...
    if (((_d = defaults.contextPruning) === null || _d === void 0 ? void 0 : _d.mode) === undefined) {
        nextDefaults.contextPruning = __assign(__assign({}, contextPruning), { mode: "cache-ttl", ttl: (_f = (_e = defaults.contextPruning) === null || _e === void 0 ? void 0 : _e.ttl) !== null && _f !== void 0 ? _f : "1h" });
        mutated = true;
    }
    if (((_g = defaults.heartbeat) === null || _g === void 0 ? void 0 : _g.every) === undefined) {
        nextDefaults.heartbeat = __assign(__assign({}, heartbeat), { every: authMode === "oauth" ? "1h" : "30m" });
        mutated = true;
    }
    if (authMode === "api_key") {
        ...
        for (var _i = 0, _m = Object.entries(nextModels); _i < _m.length; _i++) {
            var _o = _m[_i], key = _o[0], entry = _o[1];
            ...
            if (typeof params.cacheControlTtl === "string")
                continue;
            nextModels[key] = __assign(__assign({}, current), { params: __assign(__assign({}, params), { cacheControlTtl: "1h" }) });
            modelsMutated = true;
        }
        var primary = resolvePrimaryModelRef((_k = (_j = defaults.model) === null || _j === void 0 ? void 0 : _j.primary) !== null && _k !== void 0 ? _k : undefined);
        if (primary) {
            var parsedPrimary = (0, model_selection_js_1.parseModelRef)(primary, "anthropic");
            if ((parsedPrimary === null || parsedPrimary === void 0 ? void 0 : parsedPrimary.provider) === "anthropic") {
    ...
    if (!mutated)
        return cfg;
    return __assign(__assign({}, cfg), { agents: __assign(__assign({}, cfg.agents), { defaults: nextDefaults }) });
}
function applyCompactionDefaults(cfg) {
    ...
    if (compaction === null || compaction === void 0 ? void 0 : compaction.mode)
        return cfg;
    return __assign(__assign({}, cfg), { agents: __assign(__assign({}, cfg.agents), { defaults: __assign(__assign({}, defaults), { compaction: __assign(__assign({}, compaction), { mode: "safeguard" }) }) }) });
}
