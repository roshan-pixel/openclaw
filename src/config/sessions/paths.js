...
function resolveAgentSessionsDir(agentId, env, homedir) {
    ...
    var id = (0, session_key_js_1.normalizeAgentId)(agentId !== null && agentId !== void 0 ? agentId : session_key_js_1.DEFAULT_AGENT_ID);
...
function resolveSessionTranscriptPath(sessionId, agentId, topicId) {
    var safeTopicId = typeof topicId === "string"
        ? encodeURIComponent(topicId)
        : typeof topicId === "number"
            ? String(topicId)
            : undefined;
    var fileName = safeTopicId !== undefined ? "".concat(sessionId, "-topic-").concat(safeTopicId, ".jsonl") : "".concat(sessionId, ".jsonl");
...
function resolveSessionFilePath(sessionId, entry, opts) {
    var _a;
    var candidate = (_a = entry === null || entry === void 0 ? void 0 : entry.sessionFile) === null || _a === void 0 ? void 0 : _a.trim();
    return candidate ? candidate : resolveSessionTranscriptPath(sessionId, opts === null || opts === void 0 ? void 0 : opts.agentId);
}
function resolveStorePath(store, opts) {
    var _a;
    var agentId = (0, session_key_js_1.normalizeAgentId)((_a = opts === null || opts === void 0 ? void 0 : opts.agentId) !== null && _a !== void 0 ? _a : session_key_js_1.DEFAULT_AGENT_ID);
    if (!store)
        return resolveDefaultSessionStorePath(agentId);
    if (store.includes("{agentId}")) {
        var expanded = store.replaceAll("{agentId}", agentId);
        if (expanded.startsWith("~")) {
            return node_path_1.default.resolve(expanded.replace(/^~(?=$|[\\/])/, node_os_1.default.homedir()));
        }
