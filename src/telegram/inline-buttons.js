...
function resolveInlineButtonsScopeFromCapabilities(capabilities) {
    ...
    if (Array.isArray(capabilities)) {
        var enabled = capabilities.some(function (entry) { return String(entry).trim().toLowerCase() === "inlinebuttons"; });
        return enabled ? "all" : "off";
    }
    if (typeof capabilities === "object") {
        var inlineButtons = capabilities.inlineButtons;
        return (_a = normalizeInlineButtonsScope(inlineButtons)) !== null && _a !== void 0 ? _a : DEFAULT_INLINE_BUTTONS_SCOPE;
    }
    return DEFAULT_INLINE_BUTTONS_SCOPE;
}
function resolveTelegramInlineButtonsScope(params) {
    var account = (0, accounts_js_1.resolveTelegramAccount)({ cfg: params.cfg, accountId: params.accountId });
...
function isTelegramInlineButtonsEnabled(params) {
    ...
    return accountIds.some(function (accountId) { return resolveTelegramInlineButtonsScope({ cfg: params.cfg, accountId: accountId }) !== "off"; });
}
