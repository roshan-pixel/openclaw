...
function resolveTelegramStreamMode(telegramCfg) {
    var _a;
    var raw = (_a = telegramCfg === null || telegramCfg === void 0 ? void 0 : telegramCfg.streamMode) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase();
    if (raw === "off" || raw === "partial" || raw === "block")
        return raw;
    return "partial";
}
function buildTelegramGroupPeerId(chatId, messageThreadId) {
    return messageThreadId != null ? "".concat(chatId, ":topic:").concat(messageThreadId) : String(chatId);
}
...
function buildSenderName(msg) {
    var _a, _b, _c;
    var name = [(_a = msg.from) === null || _a === void 0 ? void 0 : _a.first_name, (_b = msg.from) === null || _b === void 0 ? void 0 : _b.last_name].filter(Boolean).join(" ").trim() ||
        ((_c = msg.from) === null || _c === void 0 ? void 0 : _c.username);
    return name || undefined;
}
function buildSenderLabel(msg, senderId) {
    var _a, _b;
    var name = buildSenderName(msg);
    var username = ((_a = msg.from) === null || _a === void 0 ? void 0 : _a.username) ? "@".concat(msg.from.username) : undefined;
    var label = name;
    if (name && username) {
    ...
    var normalizedSenderId = senderId != null && "".concat(senderId).trim() ? "".concat(senderId).trim() : undefined;
    var fallbackId = normalizedSenderId !== null && normalizedSenderId !== void 0 ? normalizedSenderId : (((_b = msg.from) === null || _b === void 0 ? void 0 : _b.id) != null ? String(msg.from.id) : undefined);
...
function hasBotMention(msg, botUsername) {
    ...
    var text = ((_b = (_a = msg.text) !== null && _a !== void 0 ? _a : msg.caption) !== null && _b !== void 0 ? _b : "").toLowerCase();
    if (text.includes("@".concat(botUsername)))
        return true;
    var entities = (_d = (_c = msg.entities) !== null && _c !== void 0 ? _c : msg.caption_entities) !== null && _d !== void 0 ? _d : [];
    for (var _i = 0, entities_1 = entities; _i < entities_1.length; _i++) {
        var ent = entities_1[_i];
        if (ent.type !== "mention")
            continue;
        var slice = ((_f = (_e = msg.text) !== null && _e !== void 0 ? _e : msg.caption) !== null && _f !== void 0 ? _f : "").slice(ent.offset, ent.offset + ent.length);
...
function describeReplyTarget(msg) {
    ...
    if (!body && reply) {
        var replyBody = ((_b = (_a = reply.text) !== null && _a !== void 0 ? _a : reply.caption) !== null && _b !== void 0 ? _b : "").trim();
        body = replyBody;
        if (!body) {
            if (reply.photo)
                body = "<media:image>";
    ...
    return {
        id: (reply === null || reply === void 0 ? void 0 : reply.message_id) ? String(reply.message_id) : undefined,
        sender: senderLabel,
        body: body,
        kind: kind,
    };
...
function normalizeForwardedUserLabel(user) {
    ...
    var display = (name && username
        ? "".concat(name, " (@").concat(username, ")")
        : name || (username ? "@".concat(username) : undefined)) || (id ? "user:".concat(id) : undefined);
...
function normalizeForwardedChatLabel(chat, fallbackKind) {
    ...
    var display = title || (username ? "@".concat(username) : undefined) || (id ? "".concat(fallbackKind, ":").concat(id) : undefined);
    return { display: display, title: title, username: username, id: id };
}
function buildForwardedContextFromUser(params) {
    var _a = normalizeForwardedUserLabel(params.user), display = _a.display, name = _a.name, username = _a.username, id = _a.id;
    if (!display)
        return null;
    return {
        from: display,
...
function buildForwardedContextFromChat(params) {
    var _a;
    var fallbackKind = params.type === "channel" || params.type === "legacy_channel" ? "channel" : "chat";
    var _b = normalizeForwardedChatLabel(params.chat, fallbackKind), display = _b.display, title = _b.title, username = _b.username, id = _b.id;
    if (!display)
        return null;
    var signature = ((_a = params.signature) === null || _a === void 0 ? void 0 : _a.trim()) || undefined;
...
function normalizeForwardedContext(msg) {
    var _a;
    var forwardMsg = msg;
    var signature = ((_a = forwardMsg.forward_signature) === null || _a === void 0 ? void 0 : _a.trim()) || undefined;
    ...
    if (forwardMsg.forward_from_chat) {
        var legacyType = forwardMsg.forward_from_chat.type === "channel" ? "legacy_channel" : "legacy_chat";
...
function extractTelegramLocation(msg) {
    ...
    var venue = msgWithLocation.venue, location = msgWithLocation.location;
    if (venue) {
        return {
