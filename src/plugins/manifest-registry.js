...
function resolveManifestCacheMs(env) {
    var _a;
    var raw = (_a = env.OPENCLAW_PLUGIN_MANIFEST_CACHE_MS) === null || _a === void 0 ? void 0 : _a.trim();
    if (raw === "" || raw === "0")
        return 0;
    if (!raw)
...
function shouldUseManifestCache(env) {
    var _a;
    var disabled = (_a = env.OPENCLAW_DISABLE_PLUGIN_MANIFEST_CACHE) === null || _a === void 0 ? void 0 : _a.trim();
    if (disabled)
        return false;
...
function buildRecord(params) {
    ...
    return {
        ...
        name: (_a = normalizeManifestLabel(params.manifest.name)) !== null && _a !== void 0 ? _a : params.candidate.packageName,
        description: (_b = normalizeManifestLabel(params.manifest.description)) !== null && _b !== void 0 ? _b : params.candidate.packageDescription,
        version: (_c = normalizeManifestLabel(params.manifest.version)) !== null && _c !== void 0 ? _c : params.candidate.packageVersion,
    ...
    for (var _i = 0, candidates_1 = candidates; _i < candidates_1.length; _i++) {
        ...
        if (candidate.idHint && candidate.idHint !== manifest.id) {
            diagnostics.push({
                ...
                message: "plugin id mismatch (manifest uses \"".concat(manifest.id, "\", entry hints \"").concat(candidate.idHint, "\")"),
            });
        }
        if (seenIds.has(manifest.id)) {
            diagnostics.push({
                ...
                pluginId: manifest.id,
                source: candidate.source,
                message: "duplicate plugin id detected; later plugin may be overridden (".concat(candidate.source, ")"),
            });
        }
        else {
