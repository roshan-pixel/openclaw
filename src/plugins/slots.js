...
function applyExclusiveSlotSelection(params) {
    ...
    var slots = __assign(__assign({}, pluginsConfig.slots), (_a = {}, _a[slotKey] = params.selectedId, _a));
    var inferredPrevSlot = prevSlot !== null && prevSlot !== void 0 ? prevSlot : defaultSlotIdForKey(slotKey);
    if (inferredPrevSlot && inferredPrevSlot !== params.selectedId) {
        warnings.push("Exclusive slot \"".concat(slotKey, "\" switched from \"").concat(inferredPrevSlot, "\" to \"").concat(params.selectedId, "\"."));
    }
    ...
    if (disabledIds.length > 0) {
        warnings.push("Disabled other \"".concat(slotKey, "\" slot plugins: ").concat(disabledIds.sort().join(", "), "."));
    }
    ...
    return {
        config: __assign(__assign({}, params.config), { plugins: __assign(__assign({}, pluginsConfig), { slots: slots, entries: entries }) }),
        warnings: warnings,
        changed: true,
    };
}
