...
var defaultLogger = function () { return (0, subsystem_js_1.createSubsystemLogger)("plugins"); };
var resolvePluginSdkAlias = function () {
    try {
        ...
        for (var i = 0; i < 6; i += 1) {
            ...
            for (var _i = 0, orderedCandidates_1 = orderedCandidates; _i < orderedCandidates_1.length; _i++) {
...
function buildCacheKey(params) {
    var workspaceKey = params.workspaceDir ? (0, utils_js_1.resolveUserPath)(params.workspaceDir) : "";
...
function loadOpenClawPlugins(options) {
    ...
    }), registry = _l.registry, createApi = _l.createApi;
    ...
    var jiti = (0, jiti_1.createJiti)(import.meta.url, __assign({ interopDefault: true, extensions: [".ts", ".tsx", ".mts", ".cts", ".mtsx", ".ctsx", ".js", ".mjs", ".cjs", ".json"] }, (pluginSdkAlias
        ? {
            alias: { "openclaw/plugin-sdk": pluginSdkAlias },
        }
        : {})));
    var manifestByRoot = new Map(manifestRegistry.plugins.map(function (record) { return [record.rootDir, record]; }));
    var seenIds = new Map();
    ...
    for (var _i = 0, _m = discovery.candidates; _i < _m.length; _i++) {
        ...
        var enableState = (0, config_state_js_1.resolveEnableState)(pluginId, candidate.origin, normalized);
        ...
        try {
            ...
            logger.error("[plugins] ".concat(record.id, " failed to load from ").concat(record.source, ": ").concat(String(err)));
            record.status = "error";
        ...
        if ((definition === null || definition === void 0 ? void 0 : definition.id) && definition.id !== record.id) {
            registry.diagnostics.push({
                level: "warn",
                pluginId: record.id,
                source: record.source,
                message: "plugin id mismatch (config uses \"".concat(record.id, "\", export uses \"").concat(definition.id, "\")"),
            });
        }
        record.name = (_e = definition === null || definition === void 0 ? void 0 : definition.name) !== null && _e !== void 0 ? _e : record.name;
        record.description = (_f = definition === null || definition === void 0 ? void 0 : definition.description) !== null && _f !== void 0 ? _f : record.description;
        record.version = (_g = definition === null || definition === void 0 ? void 0 : definition.version) !== null && _g !== void 0 ? _g : record.version;
        ...
        if (manifestKind && exportKind && exportKind !== manifestKind) {
            registry.diagnostics.push({
                ...
                pluginId: record.id,
                source: record.source,
                message: "plugin kind mismatch (manifest uses \"".concat(manifestKind, "\", export uses \"").concat(exportKind, "\")"),
            });
        }
        record.kind = (_h = definition === null || definition === void 0 ? void 0 : definition.kind) !== null && _h !== void 0 ? _h : record.kind;
        ...
        if (!validatedConfig.ok) {
            logger.error("[plugins] ".concat(record.id, " invalid config: ").concat((_j = validatedConfig.errors) === null || _j === void 0 ? void 0 : _j.join(", ")));
            record.status = "error";
            record.error = "invalid config: ".concat((_k = validatedConfig.errors) === null || _k === void 0 ? void 0 : _k.join(", "));
            registry.plugins.push(record);
            seenIds.set(pluginId, candidate.origin);
            registry.diagnostics.push({
                level: "error",
        ...
        try {
            ...
            logger.error("[plugins] ".concat(record.id, " failed during register from ").concat(record.source, ": ").concat(String(err)));
            record.status = "error";
            record.error = String(err);
            registry.plugins.push(record);
            seenIds.set(pluginId, candidate.origin);
