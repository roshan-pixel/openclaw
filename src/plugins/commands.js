...
function registerPluginCommand(pluginId, command) {
    ...
    if (pluginCommands.has(key)) {
        ...
        return {
            ok: false,
            error: "Command \"".concat(command.name, "\" already registered by plugin \"").concat(existing.pluginId, "\""),
        };
    }
    pluginCommands.set(key, __assign(__assign({}, command), { pluginId: pluginId }));
    (0, globals_js_1.logVerbose)("Registered plugin command: ".concat(key, " (plugin: ").concat(pluginId, ")"));
    return { ok: true };
}
/**
...
function clearPluginCommandsForPlugin(pluginId) {
    for (var _i = 0, _a = pluginCommands.entries(); _i < _a.length; _i++) {
        var _b = _a[_i], key = _b[0], cmd = _b[1];
...
function executePluginCommand(params) {
    return __awaiter(this, void 0, void 0, function () {
        var command, args, senderId, channel, isAuthorizedSender, commandBody, config, requireAuth, sanitizedArgs, ctx, result, err_1, error;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    command = params.command, args = params.args, senderId = params.senderId, channel = params.channel, isAuthorizedSender = params.isAuthorizedSender, commandBody = params.commandBody, config = params.config;
                    requireAuth = command.requireAuth !== false;
                    if (requireAuth && !isAuthorizedSender) {
                        (0, globals_js_1.logVerbose)("Plugin command /".concat(command.name, " blocked: unauthorized sender ").concat(senderId || "<unknown>"));
                ...
                case 2:
                    result = _a.sent();
                    (0, globals_js_1.logVerbose)("Plugin command /".concat(command.name, " executed successfully for ").concat(senderId || "unknown"));
                    return [2 /*return*/, result];
                case 3:
                    err_1 = _a.sent();
                    error = err_1;
                    (0, globals_js_1.logVerbose)("Plugin command /".concat(command.name, " error: ").concat(error.message));
...
function listPluginCommands() {
    return Array.from(pluginCommands.values()).map(function (cmd) { return ({
        name: cmd.name,
