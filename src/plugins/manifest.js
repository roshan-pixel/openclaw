...
function resolvePluginManifestPath(rootDir) {
    for (var _i = 0, PLUGIN_MANIFEST_FILENAMES_1 = exports.PLUGIN_MANIFEST_FILENAMES; _i < PLUGIN_MANIFEST_FILENAMES_1.length; _i++) {
...
function loadPluginManifest(rootDir) {
    ...
    if (!node_fs_1.default.existsSync(manifestPath)) {
        return { ok: false, error: "plugin manifest not found: ".concat(manifestPath), manifestPath: manifestPath };
    }
    var raw;
    try {
    ...
    if (!configSchema) {
        return { ok: false, error: "plugin manifest requires configSchema", manifestPath: manifestPath };
    }
