...
function setConsoleSubsystemFilter(filters) {
    ...
    var normalized = filters.map(function (value) { return value.trim(); }).filter(function (value) { return value.length > 0; });
...
function shouldLogSubsystemToConsole(subsystem) {
    ...
    return filter.some(function (prefix) { return subsystem === prefix || subsystem.startsWith("".concat(prefix, "/")); });
}
...
function shouldSuppressConsoleMessage(message) {
    if ((0, globals_js_1.isVerbose)())
        return false;
    if (SUPPRESSED_CONSOLE_PREFIXES.some(function (prefix) { return message.startsWith(prefix); })) {
        return true;
    }
...
function enableConsoleCapture() {
    ...
    var forward = function (level, orig) {
        return function () {
            ...
            var shouldPrefixTimestamp = state_js_1.loggingState.consoleTimestampPrefix &&
                trimmed.length > 0 &&
            ...
            if (state_js_1.loggingState.forceConsoleToStderr) {
                ...
                try {
                    ...
                    if (typeof args[0] === "string") {
                        orig.call.apply(orig, __spreadArray([console, "".concat(timestamp, " ").concat(args[0])], args.slice(1), false));
                        return;
                    }
