...
function getColorForConsole() {
    var hasForceColor = typeof process.env.FORCE_COLOR === "string" &&
    ...
    return hasTty || isRichConsoleEnv() ? new chalk_1.Chalk({ level: 1 }) : new chalk_1.Chalk({ level: 0 });
}
...
function formatConsoleLine(opts) {
    var displaySubsystem = opts.style === "json" ? opts.subsystem : formatSubsystemForConsole(opts.subsystem);
    if (opts.style === "json") {
        return JSON.stringify(__assign({ time: new Date().toISOString(), level: opts.level, subsystem: displaySubsystem, message: opts.message }, opts.meta));
    }
    var color = getColorForConsole();
    var prefix = "[".concat(displaySubsystem, "]");
    var prefixColor = pickSubsystemColor(color, displaySubsystem);
    var levelColor = opts.level === "error" || opts.level === "fatal"
        ? color.red
        : opts.level === "warn"
            ? color.yellow
...
function writeConsoleLine(level, line) {
    ...
    var sanitized = process.platform === "win32" && process.env.GITHUB_ACTIONS === "true"
...
function createSubsystemLogger(subsystem) {
    ...
    var emit = function (level, message, meta) {
        ...
        var fileMeta = meta;
        if (meta && Object.keys(meta).length > 0) {
            var _a = meta, consoleMessage_1 = _a.consoleMessage, rest = __rest(_a, ["consoleMessage"]);
        ...
        if (!(0, console_js_1.shouldLogSubsystemToConsole)(subsystem))
            return;
        var consoleMessage = consoleMessageOverride !== null && consoleMessageOverride !== void 0 ? consoleMessageOverride : message;
    ...
    var logger = {
        subsystem: subsystem,
        trace: function (message, meta) { return emit("trace", message, meta); },
        debug: function (message, meta) { return emit("debug", message, meta); },
        info: function (message, meta) { return emit("info", message, meta); },
        warn: function (message, meta) { return emit("warn", message, meta); },
        error: function (message, meta) { return emit("error", message, meta); },
        fatal: function (message, meta) { return emit("fatal", message, meta); },
        ...
        child: function (name) { return createSubsystemLogger("".concat(subsystem, "/").concat(name)); },
    };
    return logger;
}
function runtimeForLogger(logger, exit) {
    if (exit === void 0) { exit = runtime_js_1.defaultRuntime.exit; }
    return {
        log: function (message) { return logger.info(message); },
        error: function (message) { return logger.error(message); },
        exit: exit,
    };
}
