...
function resolveCommandAuthorizedFromAuthorizers(params) {
    var _a;
    var useAccessGroups = params.useAccessGroups, authorizers = params.authorizers;
    ...
    if (!useAccessGroups) {
        ...
        if (mode === "deny")
            return false;
        var anyConfigured = authorizers.some(function (entry) { return entry.configured; });
        if (!anyConfigured)
            return true;
        return authorizers.some(function (entry) { return entry.configured && entry.allowed; });
    }
    return authorizers.some(function (entry) { return entry.configured && entry.allowed; });
}
