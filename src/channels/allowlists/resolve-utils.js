...
function mergeAllowlist(params) {
    ...
    for (var _i = 0, _b = (_a = params.existing) !== null && _a !== void 0 ? _a : []; _i < _b.length; _i++) {
        var entry = _b[_i];
        push(String(entry));
    }
...
function summarizeMapping(label, mapping, unresolved, runtime) {
    ...
    if (mapping.length > 0) {
        ...
        var suffix = mapping.length > sample.length ? " (+".concat(mapping.length - sample.length, ")") : "";
        lines.push("".concat(label, " resolved: ").concat(sample.join(", ")).concat(suffix));
    }
    if (unresolved.length > 0) {
        var sample = unresolved.slice(0, 6);
        var suffix = unresolved.length > sample.length ? " (+".concat(unresolved.length - sample.length, ")") : "";
