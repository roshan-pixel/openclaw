...
function resolveChannelEntryMatchWithFallback(params) {
    ...
    if (normalizeKey) {
        var normalizedKeys = params.keys.map(function (key) { return normalizeKey(key); }).filter(Boolean);
        if (normalizedKeys.length > 0) {
            for (var _i = 0, _d = Object.entries((_a = params.entries) !== null && _a !== void 0 ? _a : {}); _i < _d.length; _i++) {
                var _e = _d[_i], entryKey = _e[0], entry = _e[1];
                var normalizedEntry = normalizeKey(entryKey);
                if (normalizedEntry && normalizedKeys.includes(normalizedEntry)) {
                    return __assign(__assign({}, direct), { entry: entry, key: entryKey, matchKey: entryKey, matchSource: "direct" });
                }
            }
        }
    }
    ...
    if (parentKeys.length > 0) {
        ...
        if (parent_1.entry && parent_1.key) {
            return __assign(__assign({}, direct), { entry: parent_1.entry, key: parent_1.key, parentEntry: parent_1.entry, parentKey: parent_1.key, matchKey: parent_1.key, matchSource: "parent" });
        }
        if (normalizeKey) {
            var normalizedParentKeys = parentKeys.map(function (key) { return normalizeKey(key); }).filter(Boolean);
            if (normalizedParentKeys.length > 0) {
                for (var _f = 0, _g = Object.entries((_c = params.entries) !== null && _c !== void 0 ? _c : {}); _f < _g.length; _f++) {
                    var _h = _g[_f], entryKey = _h[0], entry = _h[1];
                    var normalizedEntry = normalizeKey(entryKey);
                    if (normalizedEntry && normalizedParentKeys.includes(normalizedEntry)) {
                        return __assign(__assign({}, direct), { entry: entry, key: entryKey, parentEntry: entry, parentKey: entryKey, matchKey: entryKey, matchSource: "parent" });
                    }
                }
            }
        }
    ...
    if (direct.wildcardEntry && direct.wildcardKey) {
        return __assign(__assign({}, direct), { entry: direct.wildcardEntry, key: direct.wildcardKey, matchKey: direct.wildcardKey, matchSource: "wildcard" });
    }
    return direct;
}
